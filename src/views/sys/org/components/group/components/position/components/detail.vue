  ...
  >
    <template v-if="data">
      <a-card size="small" title="基本信息" class="mb-3">
        <a-row :gutter="[16, 16]">
          <a-col :xs="24" :sm="12">
            <div class="text-[13px] text-[var(--text-secondary,#00000073)] mb-1">职位编码</div>
            <div class="text-sm text-[var(--header-text,#000000d9)]">{{ data.code || '-' }}</div>
          </a-col>
          <a-col :xs="24" :sm="12">
            <div class="text-[13px] text-[var(--text-secondary,#00000073)] mb-1">职位名称</div>
            <div class="text-sm text-[var(--header-text,#000000d9)]">{{ data.name || '-' }}</div>
          </a-col>
          <a-col :xs="24" :sm="12">
            <div class="text-[13px] text-[var(--text-secondary,#00000073)] mb-1">职位类别</div>
            <div class="text-sm text-[var(--header-text,#000000d9)]">
              {{ $dict.label('POSITION_CATEGORY', data.category) }}
            </div>
          </a-col>
          <a-col :xs="24" :sm="12">
            <div class="text-[13px] text-[var(--text-secondary,#00000073)] mb-1">所属用户组</div>
            <div class="text-sm text-[var(--header-text,#000000d9)]">
              {{ groupNameMap[data.group_id] || data.group_id || '-' }}
            </div>
          </a-col>
          <a-col :xs="24" :sm="12">
...
import { fetchPositionDetail } from '@/api/position'
...
async function doOpen(row: any) {
  ...
  loading.value = true
  data.value = null
  const { data: detail } = await fetchPositionDetail({ id: row.id })
  data.value = detail
  loading.value = false
