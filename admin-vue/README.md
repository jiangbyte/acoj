...
Hei Admin Vue 与 Hei Gin、Hei Boot 等后端框架配套使用，提供完整的前后端分离架构解决方案。项目采用现代化的前端技术栈，包含完善的权限管理、丰富的 UI 组件、灵活的布局配置。

**在线文档**: [https://jiangbyte.github.io/hei-admin-vue/](https://jiangbyte.github.io/hei-admin-vue/)
...
│   ├── composables/       # 组合式函数
│   ├── hooks/             # 自定义 Hooks（useCrud、useMobile）
...
# 启动开发服务（需要先配置 .env 中的 VITE_PROXY_TARGET）
pnpm dev
...
> 开发时需确保后端服务已启动，并在 `.env` 文件中配置正确的 `VITE_PROXY_TARGET`（默认 `http://localhost:18885` 对应 Hei Gin 后端）。
