...
import { type SiteFooterInfo, emptySiteFooter, parseSiteFooter } from '@/composables/siteFooter'
...
const siteFooter = ref<SiteFooterInfo>(emptySiteFooter())
...
onMounted(async () => {
  try {
    ...
    siteFooter.value = parseSiteFooter(data)
...
async function handleSubmit() {
  ...
  const ok = await validateFields([identityItemRef.value, credentialItem])
  if (!ok) return

  if (!form.captcha_id?.trim() || !form.captcha_value.trim()) {
    const fromRef = captchaRef.value?.getValues()
    if (fromRef?.captcha_id?.trim() && fromRef.captcha_value.trim()) {
      form.captcha_id = fromRef.captcha_id
      form.captcha_value = fromRef.captcha_value
    }
  }
  ...
  try {
    ...
    const security: Record<string, string> = {
      captcha_id: form.captcha_id,
      captcha_value: form.captcha_value.trim(),
    }
    let password = ''
