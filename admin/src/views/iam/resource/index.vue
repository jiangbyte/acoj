...
async function fetchTree() {
  ...
  try {
    ...
    state.resources = stripNonMenuResources(response.data ?? [])
...
/** 资源树不展示按钮/动作（由「按钮权限」抽屉单独管理） */
function stripNonMenuResources(items: any[]): any[] {
  return (items ?? [])
    .filter((item) => item.resource_type !== 'BUTTON' && item.resource_type !== 'ACTION')
    .map((item) => ({
      ...item,
      children: stripNonMenuResources(item.children ?? []),
    }))
}
