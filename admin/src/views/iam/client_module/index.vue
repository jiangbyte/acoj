...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  },
  {
    ...
    path: 'account_type',
    width: 120,
    render: (row) => accountTypeLabel(row.account_type) || row.account_type || '-',
  },
  {
    title: '颜色',
    path: 'color',
...
async function fetchPage() {
  state.loading = true
  try {
    const response = await clientModuleApi.page({
      current: state.page,
      size: state.pageSize,
      ...state.searchValues,
    })
...
function confirmDelete(value: string | string[]) {
  ...
  window.$dialog.warning({
    ...
    draggable: true,
    maskClosable: false,
    content: isBatch ? `删除 ${ids.length} 个客户端模块?` : '删除该客户端模块?',
    positiveText: '确认',
    negativeText: '取消',
...
async function deleteData(ids: string[]) {
  await clientModuleApi.remove({ ids })
  ...
  >
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton
            v-if="hasPermission('iam:clientmodule:create')"
            type="primary"
            text
            :title="'新增'"
            :aria-label="'新增'"
          ...
          <NButton
            v-if="hasPermission('iam:clientmodule:delete')"
            type="error"
            text
            :title="'批量删除'"
            :aria-label="'批量删除'"
