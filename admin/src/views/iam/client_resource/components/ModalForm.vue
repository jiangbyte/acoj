...
import { clientModuleApi, clientResourceApi } from '@/api'
...
import { createRequiredRule, isHexColor, toNullableString, wireFields } from '@/utils'
import {
  normalizeResourceLayout,
  RESOURCE_LAYOUT_OPTIONS,
  serializeResourceLayout,
} from '@/constants/resourceLayout'
...
const layoutOptions = [...RESOURCE_LAYOUT_OPTIONS]

const modalTitle = computed(() => (state.dataId ? '编辑客户端资源' : '新增客户端资源'))

const rules = computed<FormRules>(() => ({
  code: createRequiredRule('资源编码', 'input'),
  name: createRequiredRule('资源名称', 'input'),
  resource_type: createRequiredRule('资源类型', 'change'),
  module_id: createRequiredRule('客户端模块', 'change'),
  color: [
    {
...
async function openModal(
  ...
  await Promise.all([fetchResourceTree(moduleId, accountType), fetchModules(accountType)])

  if (id) {
    await fetchDetail(id)
  } else if (parentId) {
    const parent = findResourceNode(state.resourceTree, parentId)
    state.formModel.module_id = parent?.module_id ?? moduleId ?? null
  }
}

async function fetchResourceTree(moduleId?: string | null, accountType?: AccountType | null) {
  state.treeLoading = true
  try {
    const response = await clientResourceApi.tree({
...
async function fetchModules(accountType?: AccountType | null) {
  const response = await clientModuleApi.selector(
    accountType ? { account_type: accountType } : undefined,
  )
...
async function fetchDetail(id: string) {
  state.loading = true
  try {
    const response = await clientResourceApi.detail({ id })
    state.formModel = {
      ...defaultFormData,
      ...response.data,
      ...wireFields(response.data, {
        sort: 'int',
        is_visible: 'bool',
        is_cache: 'bool',
        is_affix: 'bool',
      ...
      layout: normalizeResourceLayout(response.data?.layout),
...
async function submitForm() {
  ...
  try {
    const payload = {
      ...
      is_visible: state.formModel.is_visible,
      is_cache: state.formModel.is_cache,
      is_affix: state.formModel.is_affix,
      description: toNullableString(state.formModel.description),
      layout: serializeResourceLayout(state.formModel.layout),
      extra: state.formModel.extra ?? {},
    }

    if (state.dataId) {
      await clientResourceApi.update({
        ...payload,
        id: state.dataId,
      })
      window.$message.success('更新成功')
    } else {
      await clientResourceApi.create(payload)
