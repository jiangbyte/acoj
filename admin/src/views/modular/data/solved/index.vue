...
import { NAvatar, NButton, NCard, NDataTable, NEllipsis, NPagination, NPopconfirm, NSpace, NTag } from 'naive-ui'
...
const { dataSolvedPage, dataSolvedDelete } = useDataSolvedFetch()
const loading = ref(false)
async function loadData() {
  loading.value = true
  const { data } = await dataSolvedPage(pageParam.value)
  if (data) {
    pageData.value = data
    loading.value = false
  }
...
async function deleteHandle(row: any) {
  ...
  const { success } = await dataSolvedDelete(param)
  if (success) {
