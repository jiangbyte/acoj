...
import { dictTypeData, displayValue, formatDateTime, hasPermission, wireBool } from '@/utils'
...
async function fetchDetail(id: string) {
  ...
  state.loading = true
  try {
    const response = await sysNoticeApi.detail({ id })
...
function goEdit() {
  ...
  router.push({ path: '/sys/notice/edit', query: { id: dataId.value } })
}
    ...
    >
      <template #header-extra>
        <NSpace>
          ...
          <NButton
            v-if="hasPermission('sys:notice:update') && dataId"
            type="primary"
            @click="goEdit"
          >
            编辑
          ...
          <section class="meta-section">
            ...
            <div class="meta-grid">
              ...
              >
                ...
                <div class="meta-value">
                  {{ wireBool(state.detail.is_pinned) ? '是' : '否' }}
                </div>
              </div>
            </div>
          </section>
