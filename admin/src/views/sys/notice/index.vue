...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  },
  {
    ...
    render: (row) =>
      row.kind === 'ANNOUNCEMENT' ? (
        <NTag size="small" bordered={false} type={wireBool(row.is_pinned) ? 'warning' : 'default'}>
          {wireBool(row.is_pinned) ? '是' : '否'}
        </NTag>
      ) : (
        <span>—</span>
      ),
  ...
  },
  {
    ...
    render: (row) => (
      <NFlex size={12}>
        {hasPermission('sys:notice:detail') ? (
          <NButton type="info" text={true} onClick={() => openDetailPage(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('sys:notice:update') ? (
          <NButton type="primary" text={true} onClick={() => openEditPage(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {row.status === 'DRAFT' && hasPermission('sys:notice:publish') ? (
          <NButton type="success" text={true} onClick={() => publishRows([row.id])}>
            {renderButtonIcon('icon-park-outline:send')}
          </NButton>
        ) : null}
        {row.status === 'PUBLISHED' && hasPermission('sys:notice:revoke') ? (
          <NButton type="warning" text={true} onClick={() => revokeRows([row.id])}>
            {renderButtonIcon('icon-park-outline:return')}
          </NButton>
        ) : null}
        {row.kind === 'ANNOUNCEMENT' &&
        row.status === 'PUBLISHED' &&
        hasPermission('sys:notice:pin') ? (
          <NButton type="warning" text={true} onClick={() => togglePin(row)}>
            {renderButtonIcon(wireBool(row.is_pinned) ? 'icon-park-outline:to-bottom' : 'icon-park-outline:to-top')}
          </NButton>
        ) : null}
...
async function fetchPage() {
  state.loading = true
  try {
    const response = await sysNoticeApi.page({
      current: state.page,
      size: state.pageSize,
      ...state.searchValues,
    })
...
function openDetailPage(id: string) {
  router.push({ path: '/sys/notice/detail', query: { id } })
}

function openCreatePage() {
  router.push('/sys/notice/create')
}

function openEditPage(id: string) {
  router.push({ path: '/sys/notice/edit', query: { id } })
}
...
async function deleteRows(ids: string[]) {
  await sysNoticeApi.remove({ ids })
  state.checkedRowKeys = state.checkedRowKeys.filter((key) => !ids.includes(key))
  window.$message.success('删除成功')
  await fetchPage()
}

async function publishRows(ids: string[]) {
  await sysNoticeApi.publish({ ids })
  window.$message.success('发布成功')
  await fetchPage()
}

async function revokeRows(ids: string[]) {
  await sysNoticeApi.revoke({ ids })
  window.$message.success('撤回成功')
  await fetchPage()
}

async function togglePin(row: any) {
  const pinned = wireBool(row.is_pinned)
  await sysNoticeApi.pin({
    id: row.id,
    is_pinned: !pinned,
    pinned_until: row.pinned_until ?? null,
  })
  window.$message.success(pinned ? '已取消置顶' : '已置顶')
  await fetchPage()
}
</script>
  ...
  >
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton
            v-if="hasPermission('sys:notice:create')"
            type="primary"
            text
          ...
          <NButton
            v-if="hasPermission('sys:notice:delete')"
            type="error"
            text
