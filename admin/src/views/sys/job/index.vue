...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  },
  {
    ...
    width: 110,
    render: (row) => (
      <NTag size="small" type={row.trigger_type === 'CRON' ? 'info' : 'warning'} bordered={false}>
        {row.trigger_type === 'CRON' ? 'CRON 表达式' : '固定间隔'}
      </NTag>
    ),
  },
  {
    ...
    render: (row) => (
      <NFlex size={12}>
        {hasPermission('sys:job:run') ? (
          <NButton type="warning" text={true} onClick={() => confirmRun(row)}>
        ...
        {hasPermission('sys:joblog:page') ? (
          <NButton type="info" text={true} onClick={() => openLog(row.id)}>
            {renderButtonIcon('icon-park-outline:log')}
          </NButton>
        ) : null}
        {hasPermission('sys:job:update') ? (
          <NButton type="primary" text={true} onClick={() => openEdit(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('sys:job:update') ? (
          <NButton text={true} onClick={() => toggleEnabled(row)}>
...
function toggleEnabled(row: any) {
  ...
  window.$dialog.info({
    ...
    draggable: true,
    maskClosable: false,
    content: target ? `确认启用任务「${row.name}」？` : `确认停用任务「${row.name}」？`,
    positiveText: '确认',
    negativeText: '取消',
    onPositiveClick: async () => {
