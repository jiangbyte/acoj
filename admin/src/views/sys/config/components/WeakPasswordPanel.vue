  ...
  },
  {
    ...
    render: (row) =>
      h('div', { class: 'flex gap-12px' }, [
        hasPermission('sys:weakpassword:update')
          ? h(
              'a',
              {
          ...
          : null,
        hasPermission('sys:weakpassword:delete')
          ? h(
              'a',
              {
...
<template>
  <div class="weak-password-panel">
    ...
    >
      ...
      <NSpace>
        <NButton
          v-if="hasPermission('sys:config:update')"
          type="primary"
    ...
    <NSpace class="mb-12px">
      <NButton
        v-if="hasPermission('sys:weakpassword:create')"
        type="primary"
        @click="openCreate"
      >
        新增
      ...
      <NButton
        v-if="hasPermission('sys:weakpassword:delete')"
        type="error"
        ghost
        @click="removeBatch"
      >
