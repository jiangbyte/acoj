...
async function reload() {
  ...
  try {
    ...
    state.otpTtl = parseNumber(map.AUTH_OTP_TTL_SECONDS, 300)
...
async function save() {
  ...
  try {
    await saveByKeys([
      ...
      {
        config_key: 'AUTH_OTP_TTL_SECONDS',
        config_value: String(state.otpTtl),
        category: CATEGORY,
      },
      {
...
<template>
  <NSpin :show="state.loading">
    <ConfigSectionLayout
      description="配置会话令牌、验证码与密码重置令牌有效期，以及各端密码重置页完整 URL。"
      :saving="state.saving"
      @save="save"
      @reset="reset"
    >
      ...
      >
        ...
        <NFormItem label="验证码有效期（秒）">
          <NInputNumber
            v-model:value="state.otpTtl"
            class="w-full"
            :min="60"
          />
        </NFormItem>
