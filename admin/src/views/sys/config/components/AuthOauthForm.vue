...
const accountTabOptions = ACCOUNT_TYPE_TABS.map((item) => ({
  key: item.key,
  label: item.label,
}))
...
<template>
  <NSpin :show="state.loading">
    <NTabs
      ...
      placement="left"
      :value="state.subTab"
      @update:value="(value: string) => (state.subTab = value as AccountType)"
    >
      <NTabPane
        v-for="opt in accountTabOptions"
        :key="opt.key"
        :name="opt.key"
        :tab="opt.label"
      >
        ...
        >
          ...
          >
            ...
            <NTabs
              type="line"
              placement="left"
              class="sys-config-provider-layout"
              :value="state.providerTab"
              @update:value="(value: string) => (state.providerTab = value as ProviderKey)"
            >
              <NTabPane
