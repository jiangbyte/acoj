...
async function reload() {
  ...
  try {
    ...
    state.rows = (res.data ?? []).map((item: any) => {
      const parsed = parseSmsTemplate(item.config_value)
      return {
        id: String(item.id),
        ...
        is_builtin: wireBool(item.is_builtin),
        sort_code: wireInt(item.sort_code, 0),
...
function openEdit(row: Row) {
  ...
  state.draftCode = row.code
  state.draftContent = row.content
...
function onSceneInput(value: string) {
  ...
  if (!state.draftLabel && SMS_SCENE_LABELS[scene]) {
    state.draftLabel = SMS_SCENE_LABELS[scene]
  }
}
...
async function saveDraft() {
  ...
  state.saving = true
  try {
    const config_key = templateConfigKey('SMS_TEMPLATE', scene)
    const config_value = stringifySmsTemplate({
...
<template>
  <NSpin :show="state.loading">
    ...
    >
      <NDrawerContent
        :title="state.isCreate ? '新建短信模板' : '编辑短信模板'"
        closable
      >
        <NForm
          ref="formRef"
        ...
        >
          ...
          <NFormItem label="模板编号">
            <NInput
              v-model:value="state.draftCode"
              placeholder="云厂商模板编号"
            />
          </NFormItem>
          <NFormItem
          ...
          >
            <div class="flex flex-col gap-8px w-full min-w-0">
              <div class="w-full overflow-hidden template-monaco">
                <MonacoEditor
                  v-model:value="state.draftContent"
                  language="plaintext"
                  height="360px"
...
<style scoped>
.template-monaco :deep(.monaco-editor) {
  border: 0;
}
</style>
