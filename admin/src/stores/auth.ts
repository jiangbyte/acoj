...
const userCenterIdentityPath = '/profile?tab=identity'
...
export function isAllowedUnderSecurityWall(
  ...
  if (user.forceBindEmail || user.forceBindPhone || user.forceBindIdentity) {
    ...
    if (user.forceBindIdentity && queryTab === 'identity') return true
    return false
  }
  return true
}
...
export const useAuthStore = defineStore('auth-store', {
  ...
  actions: {
    ...
    async login(
      ...
      if (forceBindEmail || forceBindPhone || forceBindIdentity) {
    ...
    async refreshUserInfo() {
      ...
      const userInfo: AuthUserInfo = {
        ...
        identity: mapIdentity(meResponse.data.identity),
        ...
        forceBindIdentity: wireBool(meResponse.data.force_bind_identity ?? false),
