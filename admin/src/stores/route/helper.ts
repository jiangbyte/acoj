 ...
 *
 * 后端资源字段 component 存的是 views 下的相对组件路径，例如 /workspace/index.vue；
...
export function createRoutes(resources: AppRoute.RowRoute[]): RouteRecordRaw {
  ...
  const appRootRoute: RouteRecordRaw = {
    path: '/appRoot',
    name: 'appRoot',
    redirect: resolveHomePath(resources),
    component: Layout,
    meta: {},
    children: [],
  }
...
/**
 * 解析登录后首页：优先环境配置路径，否则取第一个可见 MENU。
 */
export function resolveHomePath(resources: AppRoute.RowRoute[]): string {
  const preferred = String(import.meta.env.VITE_HOME_PATH || '/workspace')
  const menus = resources.filter(
    (item) =>
      clickableResourceTypes.includes(item.resource_type) &&
      Boolean(item.path) &&
      item.is_visible !== false,
  )
  if (menus.some((item) => item.path === preferred)) {
    return preferred
  }
  const sorted = [...menus].sort((a, b) => Number(a.sort ?? 99) - Number(b.sort ?? 99))
  return sorted[0]?.path || preferred
}
