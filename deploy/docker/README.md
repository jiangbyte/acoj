...
```bash
mvn -pl oj -am package -DskipTests
```
...
## Soft sandbox (no privileged)

If the host cannot run `--privileged` / cgroup v2, edit `docker-compose.yml` judge volume:
...
1. **oj image build fails / jar missing** — run `mvn -pl oj -am package -DskipTests` first.
