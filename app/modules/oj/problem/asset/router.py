...
from app.modules.oj.problem.asset.service import OjProblemAssetService
...
async def create(
    payload: OjProblemAssetCreateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjProblemAssetService(db).create(payload)
    return success()
...
async def update(
    payload: OjProblemAssetUpdateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjProblemAssetService(db).update(payload)
    return success()
...
async def delete(
    ...
    await OjProblemAssetService(db).delete(payload)
    return success()
...
) -> ApiResponse[OjProblemAssetSchema]:
    return success(await OjProblemAssetService(db).detail(IdQuery(id=id)))
...
async def page(
...
) -> ApiResponse[PageData[OjProblemAssetSchema]]:
    query = OjProblemAssetAdminPageQuery(
    ...
    return success(await OjProblemAssetService(db).page_admin(query))
