...
from app.modules.oj.problem.objective_answer.service import OjObjectiveAnswerService
...
async def create(
    payload: OjObjectiveAnswerCreateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjObjectiveAnswerService(db).create(payload)
    return success()
...
async def update(
    payload: OjObjectiveAnswerUpdateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjObjectiveAnswerService(db).update(payload)
    return success()
...
async def delete(
    ...
    await OjObjectiveAnswerService(db).delete(payload)
    return success()
...
) -> ApiResponse[OjObjectiveAnswerSchema]:
    return success(await OjObjectiveAnswerService(db).detail(IdQuery(id=id)))
...
async def page(
...
) -> ApiResponse[PageData[OjObjectiveAnswerSchema]]:
    query = OjObjectiveAnswerAdminPageQuery(
    ...
    return success(await OjObjectiveAnswerService(db).page_admin(query))
