...
from app.modules.oj.submission.case.service import OjSubmissionCaseService
...
async def create(
    payload: OjSubmissionCaseCreateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjSubmissionCaseService(db).create(payload)
    return success()
...
async def update(
    payload: OjSubmissionCaseUpdateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjSubmissionCaseService(db).update(payload)
    return success()
...
async def delete(
    ...
    await OjSubmissionCaseService(db).delete(payload)
    return success()
...
) -> ApiResponse[OjSubmissionCaseSchema]:
    return success(await OjSubmissionCaseService(db).detail(IdQuery(id=id)))
...
async def page(
...
) -> ApiResponse[PageData[OjSubmissionCaseSchema]]:
    query = OjSubmissionCaseAdminPageQuery(
    ...
    return success(await OjSubmissionCaseService(db).page_admin(query))
