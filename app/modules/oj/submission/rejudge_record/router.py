...
from app.modules.oj.submission.rejudge_record.service import OjRejudgeRecordService
...
async def create(
    payload: OjRejudgeRecordCreateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjRejudgeRecordService(db).create(payload)
    return success()
...
async def update(
    payload: OjRejudgeRecordUpdateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjRejudgeRecordService(db).update(payload)
    return success()
...
async def delete(
    ...
    await OjRejudgeRecordService(db).delete(payload)
    return success()
...
) -> ApiResponse[OjRejudgeRecordSchema]:
    return success(await OjRejudgeRecordService(db).detail(IdQuery(id=id)))
...
async def page(
...
) -> ApiResponse[PageData[OjRejudgeRecordSchema]]:
    query = OjRejudgeRecordAdminPageQuery(
    ...
    return success(await OjRejudgeRecordService(db).page_admin(query))
