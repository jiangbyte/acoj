...
from app.modules.oj.submission.submission.service import OjSubmissionService

router = APIRouter()


@router.post(
    "/oj/submissions/create",
    dependencies=[
        Depends(require_account_type(AccountType.ADMIN)),
        Depends(require_permission("oj:submissions:create")),
    ],
    response_model=ApiResponse[None],
)
async def create(
    payload: OjSubmissionCreateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjSubmissionService(db).create(payload)
    return success()


@router.post(
    "/oj/submissions/update",
    dependencies=[
        Depends(require_account_type(AccountType.ADMIN)),
        Depends(require_permission("oj:submissions:update")),
    ],
    response_model=ApiResponse[None],
)
async def update(
    payload: OjSubmissionUpdateRequest,
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[None]:
    await OjSubmissionService(db).update(payload)
    return success()


@router.post(
    "/oj/submissions/delete",
    dependencies=[
        Depends(require_account_type(AccountType.ADMIN)),
        Depends(require_permission("oj:submissions:delete")),
    ],
    response_model=ApiResponse[None],
)
async def delete(
...
@router.get(
    "/oj/submissions/detail",
    dependencies=[
        Depends(require_account_type(AccountType.ADMIN)),
        Depends(require_permission("oj:submissions:detail")),
    ],
    response_model=ApiResponse[OjSubmissionSchema],
)
async def detail(
    db: Annotated[AsyncSession, Depends(get_db_session)],
    id: Annotated[Id, Query()],
) -> ApiResponse[OjSubmissionSchema]:
...
@router.get(
    ...
    dependencies=[
        Depends(require_account_type(AccountType.ADMIN)),
        Depends(require_permission("oj:submissions:page")),
    ],
    response_model=ApiResponse[PageData[OjSubmissionSchema]],
)
async def page(
    db: Annotated[AsyncSession, Depends(get_db_session)],
    current: Current = 1,
...
) -> ApiResponse[PageData[OjSubmissionSchema]]:
