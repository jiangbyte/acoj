...
from app.core.schema.base import CourseIdQuery, IdQuery, TeamIdQuery, to_schema, to_schema_list
...
from app.modules.biz.team.team.model import OjTeam
from app.modules.biz.team.team.repository import OjTeamRepository
...
class OjTeamService:
    ...
    async def dissolve(self, query: IdQuery, session: SessionPayload) -> None:
        team = await self.repo.get_required(query.id)
    ...
    async def search_portal_users(
        self, query: OjTeamUserSearchQuery, session: SessionPayload
    ...
    async def list_by_course_portal(self, query: CourseIdQuery, session: SessionPayload) -> list[OjTeamSchema]:
        from app.modules.biz.course.enums import CourseAccessScope
    ...
    async def disable(self, query: IdQuery) -> None:
        async with transactional(self.db):
            team = await self.repo.get_required(query.id)
            team.status = TeamStatus.DISABLED.value
            await self.repo.update_entity(team)

    async def admin_dissolve(self, query: IdQuery) -> None:
        async with transactional(self.db):
            team = await self.repo.get_required(query.id)
            team.status = TeamStatus.DISSOLVED.value
            await self.repo.update_entity(team)
    ...
    async def member_list(
        self, query: TeamIdQuery, session: SessionPayload | None = None, admin: bool = False
    ) -> list[OjTeamMemberSchema]:
    ...
    async def detail_portal(self, query: IdQuery, session: SessionPayload | None = None) -> OjTeamSchema:
        team_id = query.id
        team = await self.repo.get_required(team_id)
