...
class OjContest(Base, TimestampMixin):
    ...
    show_short_display: Mapped[bool] = mapped_column(Boolean, nullable=False, comment="展示简短设置摘要")
    problem_label_script: Mapped[str | None] = mapped_column(Text, comment="题目标签脚本")
    locked_after: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), comment="重判锁定时间")
    og_image: Mapped[str | None] = mapped_column(String(255), comment="OpenGraph 图片")
    logo_override_image: Mapped[str | None] = mapped_column(String(255), comment="Logo 覆盖图")
