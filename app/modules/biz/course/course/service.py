...
from app.core.schema.base import ClassIdQuery, CourseIdQuery, IdQuery, IdsRequest, to_schema, to_schema_list
from app.core.security.session import SessionPayload
from app.modules.biz.clazz.clazz.repository import OjClassRepository
...
from app.modules.biz.course.course.model import OjCourse, OjCourseTask
from app.modules.biz.course.course.repository import OjCourseRepository
...
class OjCourseService:
    ...
    async def publish(self, query: IdQuery) -> None:
        async with transactional(self.db):
            await self.repo.set_course_status(query.id, CourseStatus.PUBLISHED.value)

    async def archive(self, query: IdQuery) -> None:
        async with transactional(self.db):
            await self.repo.set_course_status(query.id, CourseStatus.ARCHIVED.value)

    async def list_by_class_for_member(self, query: ClassIdQuery, session: SessionPayload) -> list[OjCourseSchema]:
    ...
    async def detail_portal(
        ...
        course = await self._ensure_portal_course_read(query.id, session)
        account_id = session.account_id if session and session.account_type == AccountType.PORTAL else None
    ...
    async def detail_for_member(self, course_id: str, session: SessionPayload) -> OjCourseSchema:
        return await self.detail_portal(IdQuery(id=course_id), session)
    ...
    async def delete_announcement(self, query: IdQuery) -> None:
        async with transactional(self.db):
            await self.repo.delete_announcement(query.id)

    async def list_announcements_admin(self, query: CourseIdQuery) -> list[OjCourseAnnouncementSchema]:
        items = await self.repo.list_announcements(query.course_id, published_only=False)
        return to_schema_list(OjCourseAnnouncementSchema, items)

    async def list_announcements_portal(
        self, query: CourseIdQuery, session: SessionPayload | None = None
    ) -> list[OjCourseAnnouncementSchema]:
        await self._ensure_portal_course_read(query.course_id, session)
        items = await self.repo.list_announcements(query.course_id, published_only=True)
        return to_schema_list(OjCourseAnnouncementSchema, items)
    ...
    async def delete_task(self, query: IdQuery) -> None:
        async with transactional(self.db):
            await self.repo.delete_task(query.id)

    async def publish_task(self, query: IdQuery) -> None:
        async with transactional(self.db):
            await self.repo.set_task_status(query.id, TaskStatus.PUBLISHED.value)

    async def close_task(self, query: IdQuery) -> None:
        async with transactional(self.db):
            await self.repo.set_task_status(query.id, TaskStatus.CLOSED.value)
    ...
    async def list_tasks_admin(self, query: CourseIdQuery) -> list[OjCourseTaskSchema]:
        tasks = await self.repo.list_tasks(query.course_id, published_only=False)
    ...
    async def list_tasks_portal(
        ...
        await self._ensure_portal_course_read(query.course_id, session)
        account_id = session.account_id if session and session.account_type == AccountType.PORTAL else None
        tasks = await self.repo.list_tasks(query.course_id, published_only=True)
        return [await self._task_to_schema(t, account_id) for t in tasks]

    async def task_detail_admin(self, query: IdQuery) -> OjCourseTaskSchema:
        task = await self.repo.get_task_required(query.id)
    ...
    async def assert_can_submit(self, query: OjCourseTaskProgressBoardQuery, account_id: str) -> OjCourseTask:
    ...
    async def record_submission(self, payload: OjCourseTaskRecordSubmissionRequest, account_id: str) -> None:
        task = await self.assert_can_submit(
            OjCourseTaskProgressBoardQuery(task_id=payload.task_id), account_id
        )
