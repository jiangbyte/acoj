...
from app.modules.biz.study.learning_plan.model import OjLearningPlan, OjLearningPlanItem, OjLearningPlanSection
from app.modules.biz.study.problem_list.schema import ProblemListProblemBrief, ProblemListProgress
...
class LearningPlanService:
    ...
    async def detail_admin(self, query: IdQuery) -> LearningPlanSchema:
        return await self._to_schema(await self._get_required(query.id), with_sections=True)

    async def page_portal(self, query: LearningPlanPortalPageQuery) -> PageData[LearningPlanSchema]:
        filters = [OjLearningPlan.status == StatusEnum.ENABLED.value]
        if query.category:
            filters.append(OjLearningPlan.category == query.category)
        total = int(
