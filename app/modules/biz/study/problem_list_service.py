...
from app.modules.biz.study.model import OjProblemList, OjProblemListItem
...
class ProblemListService:
    ...
    async def admin_detail(self, list_id: str) -> ProblemListSchema:
        entity = await self._get_required(list_id)
        return await self._to_schema(entity, with_problems=True)

    # ---- portal ----
    async def mine(self, account_id: str) -> list[ProblemListSchema]:
    ...
    async def official_page(self, pagination: PageQuery) -> PageData[ProblemListSchema]:
        filters = [
    ...
    async def detail(self, list_id: str, viewer_id: str | None) -> ProblemListSchema:
        entity = await self._get_required(list_id)
    ...
    async def portal_create(self, account_id: str, payload: ProblemListCreateRequest) -> str:
        async with transactional(self.db):
    ...
    async def portal_update(self, account_id: str, payload: ProblemListUpdateRequest) -> None:
        async with transactional(self.db):
            entity = await self._get_required(payload.id)
    ...
    async def portal_delete(self, account_id: str, payload: IdsRequest) -> None:
        async with transactional(self.db):
            for lid in payload.ids:
                entity = await self._get_required(lid)
                self._assert_owner(entity, account_id)
    ...
    async def add_item(self, account_id: str, payload: ProblemListItemMutation) -> None:
        async with transactional(self.db):
    ...
    async def remove_item(self, account_id: str, payload: ProblemListItemMutation) -> None:
        async with transactional(self.db):
            entity = await self._get_required(payload.list_id)
            self._assert_owner(entity, account_id)
            await self.db.execute(
    ...
    async def is_favorited(self, account_id: str, problem_id: str) -> bool:
        fav = await self.ensure_favorites(account_id)
        exists = (
            await self.db.execute(
                select(OjProblemListItem.id).where(
                    OjProblemListItem.list_id == fav.id,
                    OjProblemListItem.problem_id == problem_id,
                )
            )
        ).scalar_one_or_none()
        return exists is not None

    async def add_favorite(self, account_id: str, problem_id: str) -> None:
        fav = await self.ensure_favorites(account_id)
        await self.add_item(account_id, ProblemListItemMutation(list_id=fav.id, problem_id=problem_id))

    async def remove_favorite(self, account_id: str, problem_id: str) -> None:
        fav = await self.ensure_favorites(account_id)
        await self.remove_item(account_id, ProblemListItemMutation(list_id=fav.id, problem_id=problem_id))

    async def reorder(self, account_id: str, payload: ProblemListReorderRequest) -> None:
        async with transactional(self.db):
            entity = await self._get_required(payload.list_id)
            self._assert_owner(entity, account_id)
