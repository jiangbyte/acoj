...
from app.core.schema.base import ClassIdQuery, IdQuery, IdsRequest
...
from app.modules.biz.clazz.clazz.schema import (
...
from app.modules.biz.clazz.clazz.service import OjClassService
...
async def admin_page(
    query: Annotated[OjClassAdminPageQuery, Depends()],
    db: Annotated[AsyncSession, Depends(get_db_session)],
) -> ApiResponse[PageData[OjClassSchema]]:
    return success(await OjClassService(db).page_admin(query))
...
async def admin_detail(
    ...
    return success(await OjClassService(db).detail(query))
...
async def admin_members(
    ...
    return success(await OjClassService(db).member_list(query, admin=True))
...
async def portal_detail(
    ...
    return success(await OjClassService(db).detail_portal(query, session))
...
async def portal_members(
    ...
    return success(await OjClassService(db).member_list(query, session=session))
