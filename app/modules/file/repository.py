...
from app.modules.file.schema import FileRecordCreate
...
class FileRepository:
    ...
    async def list_files(self, offset: int, limit: int) -> list[SysFile]:
        """分页查询文件元数据列表。"""
        stmt = select(SysFile).order_by(SysFile.created_at.desc()).offset(offset).limit(limit)
