...
class MessageRepository:
    ...
    async def create_message(self, payload: SendMessageRequest, conversation_id: str,
                             sender_account_type: str | None, sender_account_id: str | None,
                             sender_type: str = "USER") -> MsgMessage:
        ...
        for i, att in enumerate(payload.attachments):
            self.db.add(MsgMessageAttachment(
                id=generate_snowflake_id(),
                message_id=msg.id,
                file_id=att.file_id,
                name=att.name,
    ...
    async def page_messages(self, conversation_id: str, offset: int, size: int) -> tuple[list[MsgMessage], int]:
        stmt = (
            select(MsgMessage)
        ...
        total = (await self.db.execute(
            select(func.count(MsgMessage.id)).where(MsgMessage.conversation_id == conversation_id)
        )).scalar_one()
        return items, total

    async def map_attachments(self, message_ids: list[str]) -> dict[str, list[MsgMessageAttachment]]:
        if not message_ids:
            return {}
        stmt = select(MsgMessageAttachment).where(
            MsgMessageAttachment.message_id.in_(list(dict.fromkeys(message_ids)))
    ...
    async def get_message_before(self, conversation_id: str, message_id: str) -> MsgMessage | None:
        ...
        before = await self.db.execute(
            select(MsgMessage).where(
                MsgMessage.conversation_id == conversation_id,
                MsgMessage.id < message_id
            ).order_by(MsgMessage.id.desc()).limit(1)
        )
    ...
    async def count_unread(self, conversation_id: str, account_type: str, account_id: str) -> int:
        ...
        cursor = await self.db.execute(
            ...
            ).order_by(MsgMessageRead.last_read_at.desc()).limit(1)
        )
        read = cursor.scalar_one_or_none()
        if read is None:
            stmt = select(func.count(MsgMessage.id)).where(MsgMessage.conversation_id == conversation_id)
        else:
    ...
    async def mark_read(self, conversation_id: str, account_type: str, account_id: str,
                        last_read_message_id: str, terminal_id: str | None = None) -> None:
        now = datetime.now(timezone.utc)
        stmt = select(MsgMessageRead).where(
            MsgMessageRead.conversation_id == conversation_id,
            MsgMessageRead.account_type == account_type,
            MsgMessageRead.account_id == account_id,
            MsgMessageRead.terminal_id == terminal_id if terminal_id else MsgMessageRead.terminal_id.is_(None),
        )
