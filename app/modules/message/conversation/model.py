...
from sqlalchemy import JSON, Boolean, DateTime, Float, Integer, Numeric, String, Text, UniqueConstraint
...
class MsgConversation(Base, TimestampMixin):
    ...
    conversation_type: Mapped[str] = mapped_column(String(32), nullable=False, comment="会话类型 DIRECT/GROUP")
    ...
    last_message_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), comment="最新消息时间")
...
class MsgConversationMember(Base, TimestampMixin):
    ...
    __table_args__ = (
        UniqueConstraint("conversation_id", "account_type", "account_id", name="uq_conversation_member"),
    )
    ...
    role: Mapped[str] = mapped_column(String(32), nullable=False, default="MEMBER", comment="角色 OWNER/MEMBER")
    unread_count: Mapped[int] = mapped_column(Integer, nullable=False, default=0, comment="未读消息数")
    last_read_message_id: Mapped[str | None] = mapped_column(String(64), comment="最后已读消息ID")
    last_read_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), comment="最后已读时间")
    last_delivered_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), comment="最后投递时间")
    is_muted: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False, comment="是否免打扰")
