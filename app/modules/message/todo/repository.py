...
class TodoRepository:
    ...
    async def create_todo(
        ...
        *,
        ...
        if (
            payload.target_scope == MessageTargetScope.SPECIFIC
            and payload.target_account_type
            and payload.target_account_id
        ):
            refs.append(
                AccountRef(
                    account_type=payload.target_account_type, account_id=payload.target_account_id
                )
            )
    ...
    async def cancel_todo(self, todo_id: str) -> None:
        ...
        await self.db.execute(
            ...
            .where(
                ...
                MsgTodoAssignee.status != TodoAssigneeStatus.COMPLETED.value,
            )
    ...
    async def delete_todos(self, ids: list[str]) -> None:
        ...
        await self.db.execute(
            delete(MsgTodoAssignee).where(MsgTodoAssignee.todo_id.in_(unique_ids))
        )
                ...
                await self.db.execute(
                    select(MsgTodoAssignee.account_type, MsgTodoAssignee.account_id).where(
                        ...
                        tuple_account_filter(
                            MsgTodoAssignee.account_type, MsgTodoAssignee.account_id, unique
                        ),
                    )
                )
            ).all()
    ...
    async def page_todos_admin(self, query: TodoAdminPageQuery) -> tuple[list[MsgTodo], int]:
        ...
        stmt = (
            stmt.order_by(MsgTodo.updated_at.desc(), MsgTodo.id.desc())
    ...
    async def page_my_todos(
        ...
        *,
        ...
        stmt = (
            stmt.order_by(MsgTodo.due_at.asc().nullslast(), MsgTodo.updated_at.desc())
            .offset(query.pagination.offset)
            .limit(query.pagination.size)
        )
    ...
    async def get_my_todo(
        ...
        *,
        ...
        stmt = (
            select(MsgTodo, MsgTodoAssignee.status)
            .outerjoin(
                MsgTodoAssignee,
                and_(
                    MsgTodoAssignee.todo_id == MsgTodo.id,
    ...
    async def count_my_todos(self, *, account_type: str, account_id: str) -> int:
        ...
        stmt = stmt.where(
            ...
            or_(
                MsgTodoAssignee.status.is_(None),
                MsgTodoAssignee.status == TodoAssigneeStatus.PENDING.value,
            ),
        )
    ...
    async def update_my_todo_status(
        ...
        *,
        ...
        if assignee is None:
            ...
            assignee = MsgTodoAssignee(
                todo_id=todo_id, account_type=account_type, account_id=account_id
            )
    ...
    def _todo_visible_filter(self, account_type: str, account_id: str):
        return or_(
            and_(
                ...
                or_(
                    MsgTodo.target_account_type.is_(None),
                    MsgTodo.target_account_type == account_type,
                ),
            ),
            and_(
                ...
                or_(
                    and_(
                        MsgTodo.target_account_type == account_type,
                        MsgTodo.target_account_id == account_id,
                    ),
                    and_(
                        MsgTodoAssignee.account_type == account_type,
                        MsgTodoAssignee.account_id == account_id,
