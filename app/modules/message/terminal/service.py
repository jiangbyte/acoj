...
class MsgTerminalService:
    ...
    async def page_admin(self, query: MsgTerminalAdminPageQuery) -> PageData[MsgTerminalSchema]:
        ...
        return build_page(query.pagination, total, to_schema_list(MsgTerminalSchema, items))

    async def register(self, payload: TerminalRegisterRequest, session: SessionPayload) -> MsgTerminalSchema:
        async with transactional(self.db):
    ...
    async def unregister(self, entity_id: str, session: SessionPayload) -> None:
        async with transactional(self.db):
            ...
            if entity.account_type != str(session.account_type) or entity.account_id != session.account_id:
                raise NotFoundError("MsgTerminal not found")
            await self.repo.delete_many([entity_id])

    async def update_push_token(self, payload: PushTokenUpdateRequest, session: SessionPayload) -> None:
        async with transactional(self.db):
