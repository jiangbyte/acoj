package io.charlie.web.modular.data.problem.importdata;// FileImportService.java

import cn.hutool.core.util.StrUtil;
import io.charlie.web.modular.data.problem.entity.DataProblem;
import io.charlie.web.modular.data.problem.mapper.DataProblemMapper;
import io.charlie.web.modular.data.problem.param.DataProblemImportParam;
import io.charlie.web.modular.data.relation.tag.service.DataProblemTagService;
import io.charlie.web.modular.data.testcase.entity.DataTestCase;
import io.charlie.web.modular.data.testcase.mapper.DataTestCaseMapper;
import io.charlie.web.modular.data.problem.utils.FpsXmlParser;
...
public class FileImportService {
    ...
    public ImportResult importFpsFile(DataProblemImportParam dataProblemImportParam) {
        try {
            String fileName = Objects.requireNonNull(dataProblemImportParam.getFile().getOriginalFilename()).toLowerCase();

            if (fileName.endsWith(".xml")) {
                // 单个XML文件导入
                return importSingleXml(dataProblemImportParam);
            } else if (fileName.endsWith(".zip")) {
                // ZIP压缩包导入
                return importZipFile(dataProblemImportParam);
            } else {
                return ImportResult.error("不支持的文件格式，请上传XML或ZIP文件");
            }
        } catch (Exception e) {
    ...
    private ImportResult importSingleXml(DataProblemImportParam dataProblemImportParam) throws IOException, DocumentException {
        try (InputStream inputStream = dataProblemImportParam.getFile().getInputStream()) {
            List<ProblemImportData> problemDataList = fpsXmlParser.parseFpsXml(inputStream);
            return saveProblems(problemDataList, dataProblemImportParam);
        }
    }
     ...
     * 导入ZIP压缩包 - 修复版本
     */
    private ImportResult importZipFile(DataProblemImportParam dataProblemImportParam) throws IOException {
        List<ProblemImportData> allProblemData = new ArrayList<>();
        List<String> zipErrors = new ArrayList<>();

        try (ZipArchiveInputStream zis = new ZipArchiveInputStream(dataProblemImportParam.getFile().getInputStream())) {
            ZipArchiveEntry entry;
        ...
        ImportResult result = saveProblems(allProblemData, dataProblemImportParam);
        // 添加ZIP处理过程中的错误信息
        if (!zipErrors.isEmpty()) {
            result.getErrors().addAll(0, zipErrors);
            result.setMessage(result.getMessage() + " (部分文件解析失败)");
     ...
     * 保存题目数据到数据库
     */
    private ImportResult saveProblems(List<ProblemImportData> problemDataList, DataProblemImportParam dataProblemImportParam) {
        int successCount = 0;
        int totalCount = problemDataList.size();
        List<String> errorMessages = new ArrayList<>();
        ...
        for (ProblemImportData importData : problemDataList) {
            try {
                ...
                problem.setDifficulty(dataProblemImportParam.getDifficulty());
                problem.setAllowedLanguages(dataProblemImportParam.getAllowedLanguages());
                dataProblemMapper.insert(problem);

                // 处理标签
                dataProblemTagService.addOrUpdate(problem.getId(), dataProblemImportParam.getTagIds());
