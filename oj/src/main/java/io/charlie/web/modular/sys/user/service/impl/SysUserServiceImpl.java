package io.charlie.web.modular.sys.user.service.impl;
...
import io.charlie.cores.utils.str.GalaxyStringUtil;
import io.charlie.web.context.DataScopeUtil;
...
import io.charlie.web.modular.sys.auth.utils.UserValidationUtil;
import io.charlie.web.modular.sys.config.service.SysConfigService;
import io.charlie.web.modular.sys.relation.entity.SysUserRole;
import io.charlie.web.modular.sys.relation.mapper.SysUserRoleMapper;
import io.charlie.web.modular.sys.relation.service.SysUserRoleService;
import io.charlie.web.constant.ERoleConstant;
import io.charlie.web.constant.EUserConstant;
...
import io.charlie.web.modular.sys.user.entity.ACRecord;
import io.charlie.web.modular.sys.user.entity.SysNoNeUser;
import io.charlie.web.modular.sys.user.entity.SysUser;
import io.charlie.web.modular.sys.user.param.*;
import io.charlie.web.modular.sys.user.mapper.SysUserMapper;
import io.charlie.web.modular.sys.user.service.SysUserService;
import io.charlie.cores.enums.ISortOrderEnum;
...
import io.charlie.web.modular.sys.user.utils.SysUserBuildUtil;
...
//        UserValidationUtil.validatePassword(password).throwIfFailed();
        ...
        if (GalaxyStringUtil.isEmpty(bean.getAvatar())) {
            ...
            bean.setAvatar(toStoredObjectName(bean.getAvatar()));
        }
        if (GalaxyStringUtil.isEmpty(bean.getBackground())) {
            ...
            bean.setBackground(toStoredObjectName(bean.getBackground()));
        }
    ...
    public void edit(SysUserEditParam sysUserEditParam) {
        ...
        if (StrUtil.isNotBlank(bean.getAvatar())) {
            bean.setAvatar(toStoredObjectName(bean.getAvatar()));
        }
        if (StrUtil.isNotBlank(bean.getBackground())) {
            bean.setBackground(toStoredObjectName(bean.getBackground()));
        }
    ...
    public Boolean updateAvatar(SysUserUpdateImageParam sysUserUpdateImageParam) {
        ...
        return this.update(new LambdaUpdateWrapper<SysUser>()
                ...
                .set(SysUser::getAvatar, toStoredObjectName(sysUserUpdateImageParam.getImg()))
        );
    }
    ...
    public Boolean updateBackground(SysUserUpdateImageParam sysUserUpdateImageParam) {
        ...
        return this.update(new LambdaUpdateWrapper<SysUser>()
                .eq(SysUser::getId, sysUserUpdateImageParam.getId())
                .set(SysUser::getBackground, toStoredObjectName(sysUserUpdateImageParam.getImg()))
        );
    }
     ...
     * 库中只存对象名（外链原样保留）
     */
    private String toStoredObjectName(String value) {
        if (StrUtil.isBlank(value)) {
            return value;
        }
        StorageService storageService = storageServiceFactory.getStorageService();
        return storageService.toObjectName(value);
    }
