...
onMounted(async () => {
  ...
  for (const row of res.data ?? []) {
    switch (row.config_key) {
      case 'mail.host': fields.host = { id: row.id, config_key: row.config_key, value: row.config_value ?? '', remark: row.remark ?? '' }; break
      case 'mail.port': fields.port = { id: row.id, value: Number(row.config_value) || 0, remark: row.remark ?? '' }; break
      case 'mail.username': fields.username = { id: row.id, config_key: row.config_key, value: row.config_value ?? '', remark: row.remark ?? '' }; break
      case 'mail.password': fields.password = { id: row.id, config_key: row.config_key, value: row.config_value ?? '', remark: row.remark ?? '' }; break
      case 'mail.from_email': fields.fromEmail = { id: row.id, config_key: row.config_key, value: row.config_value ?? '', remark: row.remark ?? '' }; break
      case 'mail.from_name': fields.fromName = { id: row.id, config_key: row.config_key, value: row.config_value ?? '', remark: row.remark ?? '' }; break
      case 'mail.use_tls': fields.useTls = { id: row.id, value: row.config_value === 'true', remark: row.remark ?? '' }; break
    }
  }
})
...
function buildPayload() {
  return {
    items: [
      ...
      { id: fields.fromEmail.id, config_key: 'mail.from_email', config_value: fields.fromEmail.value },
      { id: fields.fromName.id, config_key: 'mail.from_name', config_value: fields.fromName.value },
      { id: fields.useTls.id, config_key: 'mail.use_tls', config_value: String(fields.useTls.value) },
    ],
  }
}
      ...
      <NGi :span="16">
        <NFormItem label="SMTP 地址" :style="{ marginBottom: 0 }">
          <div class="w-full">
            ...
            <div class="hint">{{ fields.host.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
      <NGi :span="8">
        <NFormItem label="SMTP 端口" :style="{ marginBottom: 0 }">
          <div class="w-full">
            ...
            <div class="hint">{{ fields.port.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
      <NGi :span="12">
        <NFormItem label="SMTP 用户名" :style="{ marginBottom: 0 }">
          <div class="w-full">
            ...
            <div class="hint">{{ fields.username.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
      <NGi :span="12">
        <NFormItem label="SMTP 密码" :style="{ marginBottom: 0 }">
          <div class="w-full">
            <NInput v-model:value="fields.password.value" type="password" show-password-on="click" />
            <div class="hint">{{ fields.password.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
      <NGi :span="12">
        <NFormItem label="发件人邮箱" :style="{ marginBottom: 0 }">
          <div class="w-full">
            ...
            <div class="hint">{{ fields.fromEmail.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
      <NGi :span="12">
        <NFormItem label="发件人名称" :style="{ marginBottom: 0 }">
          <div class="w-full">
            ...
            <div class="hint">{{ fields.fromName.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
      <NGi :span="8">
        <NFormItem label="TLS 加密" :style="{ marginBottom: 0 }">
          <div>
            ...
            <div class="hint">{{ fields.useTls.remark }}</div>
          </div>
        </NFormItem>
      </NGi>
    </NGrid>
