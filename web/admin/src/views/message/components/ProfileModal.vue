...
<template>
  <NModal v-model:show="ui.showProfileModal.value" preset="card" draggable :bordered="false" title="个人信息"
    style="width: min(460px, calc(100vw - 32px));">
    <div class="flex flex-col gap-4">
      <div class="flex items-center gap-3">
        <NAvatar v-if="avatarUrl" round :size="56" :src="avatarUrl" :img-props="avatarImgProps" class="shrink-0" />
        ...
        <div class="min-w-0">
          ...
          <div class="mt-1 text-xs" :style="{ color: themeVars.textColor3 }">{{ data.profile?.title }}</div>
        </div>
      </div>
      <NDescriptions :column="1" label-placement="left" size="small">
        <NDescriptionsItem label="账号">{{ data.profile?.account }}</NDescriptionsItem>
        <NDescriptionsItem label="角色">{{ data.profile?.role }}</NDescriptionsItem>
        <NDescriptionsItem label="部门">{{ data.profile?.department }}</NDescriptionsItem>
      </NDescriptions>
