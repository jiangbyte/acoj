  ...
  <div class="flex h-full min-h-0 flex-col">
    <NScrollbar class="h-full">
      <div class="flex flex-col">
        ...
        <div class="flex flex-col items-center px-4 pb-6 pt-8 text-white"
          :style="{ backgroundColor: themeVars.primaryColor }">
          <NAvatar v-if="avatarUrl" round :size="80" :src="avatarUrl" :img-props="avatarImgProps"
            class="shrink-0 border-2 shadow-lg" :style="{ borderColor: 'rgba(255,255,255,0.3)' }" />
          <NAvatar v-else round :size="80"
            class="shrink-0 border-2 shadow-lg" :style="{ borderColor: 'rgba(255,255,255,0.3)' }">
            <NovaIcon icon="icon-park-outline:user" :size="40" />
          </NAvatar>
          <div class="mt-3 text-lg font-600">{{ data.profile?.nickname }}</div>
          <div v-if="data.profile?.title || data.profile?.department" class="mt-0.5 text-sm" style="color: rgba(255,255,255,0.7)">
            {{ data.profile?.title }}<template v-if="data.profile?.title && data.profile?.department"> · </template>{{ data.profile?.department }}
          </div>
        </div>

        <div class="flex flex-col gap-0 px-4 py-3" :style="{ backgroundColor: themeVars.cardColor }">
          <div class="flex items-center justify-between border-b py-4" :style="{ borderColor: themeVars.borderColor }">
