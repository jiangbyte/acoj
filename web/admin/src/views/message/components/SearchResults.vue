...
const filteredGroups = computed(() => {
  ...
  return data.groups.filter((g) =>
    g.name.toLowerCase().includes(k) || (g.description ?? '').toLowerCase().includes(k)
  )
})
...
<template>
  <div class="flex min-h-0 flex-1 flex-col">
    <NTabs v-model:value="ui.searchScope.value" type="segment" size="small" class="px-4 pt-3" @update:value="doSearchUsers">
      <NTabPane name="conversations" tab="对话">
        <NScrollbar class="h-full">
          <NList v-if="filteredConversations.length" hoverable clickable>
            <NListItem v-for="conv in filteredConversations" :key="conv.id" class="message-list-item cursor-pointer"
              ...
              <div class="message-list-row flex items-start gap-3 px-4 py-3">
                ...
                <div class="message-list-body">
                  <div class="message-list-main-line flex items-center justify-between gap-3">
                    <span class="message-ellipsis flex-1 text-sm font-600">{{ conv.title || '会话' }}</span>
                    <span class="shrink-0 text-xs" :style="{ color: themeVars.textColor3 }">{{ formatDateTime(conv.last_message_at ?? conv.created_at) }}</span>
                  </div>
                </div>
              </div>
            </NListItem>
      ...
      <NTabPane name="users" tab="用户">
        <NScrollbar class="h-full">
          <NList v-if="searchResultUsers.length" hoverable clickable>
            <NListItem v-for="user in searchResultUsers" :key="`${user.account_type}-${user.account_id}`" class="message-list-item">
              <div class="message-list-row flex items-start gap-3 px-4 py-3">
                <NAvatar v-if="user.avatar" round :size="40" class="shrink-0" :src="resolveFileUrl(user.avatar)" :img-props="avatarImgProps" />
                <NAvatar v-else round :size="40" class="shrink-0">{{ (user.name || user.nickname || '?').charAt(0) }}</NAvatar>
                <div class="message-list-body">
                  <div class="message-list-main-line flex items-center justify-between gap-3">
                    <span class="message-ellipsis flex-1 text-sm font-600">{{ user.name || user.nickname }}</span>
                  </div>
                  <span class="message-ellipsis mt-1 text-xs" :style="{ color: themeVars.textColor3 }">{{ user.signature || '-' }}</span>
                </div>
                <NTag v-if="user.is_friend" :bordered="false" size="small" type="success">好友</NTag>
              </div>
            </NListItem>
          </NList>
          <div v-else-if="searchLoading" class="py-8 text-center text-sm" :style="{ color: themeVars.textColor3 }">搜索中...</div>
      ...
      <NTabPane name="groups" tab="群组">
        <NScrollbar class="h-full">
          <NList v-if="filteredGroups.length" hoverable clickable>
            <NListItem v-for="group in filteredGroups" :key="group.id" class="message-list-item cursor-pointer"
