...
watch(() => props.draft, (d) => {
  if (d) { composerText.value = d.text; selectedAttachments.value = d.attachments; selectedFiles.value = [] }
}, { immediate: false })
...
async function loadOlderMessages() {
  ...
  finally { messageState.loadingOlder = false }
}
...
async function loadMessages() {
  ...
  finally { messageState.loading = false }
}
...
function isOwnMessage(message: Message): boolean {
  return message.sender_account_id === data.profile?.account_id
  ...
  <div v-if="conversation" class="flex h-full min-h-0 flex-col">
    <div class="flex items-center justify-between gap-3 border-b px-4 py-3" :style="{ borderColor: themeVars.borderColor }">
      <div class="flex min-w-0 items-center gap-3 overflow-hidden">
        ...
        <NAvatar v-if="conversation.avatar" round :size="42" class="shrink-0" :src="resolveFileUrl(conversation.avatar)" :img-props="avatarImgProps" />
        <NAvatar v-else round :size="42" class="shrink-0">{{ (conversation.title || '?').charAt(0) }}</NAvatar>
        <NThing :title="conversation.title" :description="conversation.conversation_type === 'GROUP' ? '群聊' : '私聊'" />
      </div>
      <NFlex :size="4">
    ...
    <div class="flex min-h-0 flex-1 flex-col">
      ...
      <div v-if="messageState.loading && !visibleMessages.length" class="flex items-center justify-center py-12 text-sm" :style="{ color: themeVars.textColor3 }">
        加载中...
      </div>
