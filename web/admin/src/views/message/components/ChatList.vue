...
function getConversationPreview(convId: string, conversation: any): string {
  ...
  if (msgs?.length) {
    ...
    const prefix = latest.sender_nickname || latest.sender_name ? `${latest.sender_nickname || latest.sender_name}：` : ''
    return `${prefix}${latest.content}`
  }
  return conversation.title ?? ''
}

function conversationAvatarText(conv: any): string {
  if (conv.title) return conv.title.charAt(0)
  return '?'
}
</script>
  ...
  <div class="flex min-h-0 flex-1 flex-col">
    <NScrollbar class="h-full">
      <NList v-if="sortedConversations.length" hoverable clickable>
        <NListItem v-for="conv in sortedConversations" :key="conv.id" class="message-list-item cursor-pointer"
          :style="conv.id === (data as any).selectedConversationId ? { backgroundColor: themeVars.buttonColor2Hover } : {}"
          ...
          <div class="message-list-row flex items-start gap-3 px-4 py-3">
            ...
            <div class="message-list-body">
              ...
              <span class="message-ellipsis mt-1 text-xs" :style="{ color: themeVars.textColor3 }">{{ getConversationPreview(conv.id, conv) }}</span>
            </div>
          </div>
        </NListItem>
      </NList>
