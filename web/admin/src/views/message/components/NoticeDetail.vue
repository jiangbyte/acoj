        ...
        <NScrollbar class="h-full">
          <div class="mx-auto flex w-full max-w-[460px] flex-col gap-4 px-4 py-6">
            ...
            <div class="flex items-center gap-3" style="position:relative;overflow:visible">
              <NAvatar v-if="avatarUrl" round :size="64" class="shrink-0" :src="avatarUrl" :img-props="avatarImgProps" />
              <NAvatar v-else round :size="64" class="shrink-0">{{ (props.request?.applicant_name || '?').charAt(0) }}</NAvatar>
              <div v-if="request && request.status !== 'PENDING'"
                ...
                :style="request.status === 'ACCEPTED' ? 'color:#18a058;border-color:#18a058;' : 'color:#d03050;border-color:#d03050;'">
                {{ request.status === 'ACCEPTED' ? '已通过' : '已拒绝' }}
              </div>
              <div class="min-w-0 text-left">
                <div class="truncate text-lg font-600">{{ 'applicant_name' in request ? request.applicant_name : '申请' }}</div>
                <div class="truncate text-xs" :style="{ color: themeVars.textColor3 }">{{ 'group_name' in request ? request.group_name : '好友申请' }}</div>
              </div>
            </div>
            <NDescriptions :column="1" label-placement="left" size="small">
              <NDescriptionsItem label="类型">{{ 'group_name' in request ? '入群申请' : '好友申请' }}</NDescriptionsItem>
              <NDescriptionsItem label="说明">{{ 'message' in request ? request.message : '' }}</NDescriptionsItem>
              <NDescriptionsItem label="时间">{{ formatDateTime(request.created_at) }}</NDescriptionsItem>
            </NDescriptions>
            <NFlex justify="center" :wrap="true" :size="12">
              ...
              <NTag v-else :bordered="false" size="small">{{ request?.status === 'ACCEPTED' ? '已通过' : request?.status === 'REJECTED' ? '已拒绝' : '待处理' }}</NTag>
            </NFlex>
          </div>
        </NScrollbar>
      </div>
          ...
          <div class="mx-auto flex w-full max-w-[460px] flex-col gap-4 px-4 py-6">
            ...
            <div class="flex items-center gap-3">
              ...
              <div class="min-w-0 text-left">
                ...
                <div class="truncate text-xs" :style="{ color: themeVars.textColor3 }">{{ formatDateTime(notice.created_at) }}</div>
              </div>
            </div>
