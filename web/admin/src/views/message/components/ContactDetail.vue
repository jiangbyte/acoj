...
const avatarUrl = computed(() => resolveFileUrl(props.friend?.avatar) || resolveFileUrl(props.group?.avatar) || undefined)
        ...
        <NScrollbar class="h-full">
          <div class="mx-auto flex w-full max-w-[460px] flex-col gap-4 px-4 py-6">
            ...
            <div class="flex items-center gap-3">
              ...
              <NAvatar v-else round :size="64" class="shrink-0">{{ (friend?.name || group?.name || '?').charAt(0) }}</NAvatar>
              <div class="min-w-0 text-left">
                <div class="truncate text-lg font-600">{{ friend?.name || group?.name }}</div>
                <div class="truncate text-xs" :style="{ color: themeVars.textColor3 }">{{ friend ? (friend.signature || '-') : (group?.description || '-') }}</div>
              </div>
            </div>
            <NDescriptions :column="1" label-placement="left" size="small">
              <template v-if="friend">
                <NDescriptionsItem label="备注">{{ friend.remark || '-' }}</NDescriptionsItem>
              ...
              <template v-else-if="group">
                ...
                <NDescriptionsItem label="状态">{{ group.status }}</NDescriptionsItem>
              </template>
            </NDescriptions>
