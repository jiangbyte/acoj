...
const filteredInviteFriends = computed(() => {
  ...
  return data.friends.filter((f) =>
    (f.name ?? '').toLowerCase().includes(keyword) || (f.nickname ?? '').toLowerCase().includes(keyword)
  )
})
...
async function handleCreateGroup() {
  ...
  try {
    const groupRes = await messageApi.createGroup({ name, description: createGroupDesc.value.trim() || undefined })
    // Reload groups
  ...
  >
    <div class="flex flex-col gap-4">
      <div class="flex items-center gap-4">
        <NAvatar round :size="56" class="shrink-0 border-2 border-dashed" :style="{ borderColor: 'var(--border-color)' }">
    ...
    <div class="flex min-h-0 flex-1 flex-col gap-3">
      ...
      <NScrollbar class="flex-1" style="max-height: 40vh;">
        <NList v-if="filteredInviteFriends.length" hoverable>
          <NListItem v-for="friend in filteredInviteFriends" :key="friend.friendship_id" class="message-list-item cursor-pointer" @click="toggleGroupInvitee(friend.friendship_id)">
            <div class="flex items-center gap-3 px-2 py-2">
              <NCheckbox :checked="createGroupInvitees.includes(friend.friendship_id)" @click.stop="toggleGroupInvitee(friend.friendship_id)" />
              <NAvatar v-if="friend.avatar" round :size="36" class="shrink-0" :src="resolveFileUrl(friend.avatar)" :img-props="avatarImgProps" />
              <NAvatar v-else round :size="36" class="shrink-0">{{ (friend.name || friend.nickname || '?').charAt(0) }}</NAvatar>
              <div class="min-w-0 flex-1">
                <div class="message-ellipsis text-sm font-500">{{ friend.name || friend.nickname }}</div>
                <div class="message-ellipsis text-xs" style="color: var(--text-color-3)">{{ friend.signature || '-' }}</div>
              </div>
            </div>
          </NListItem>
        </NList>
