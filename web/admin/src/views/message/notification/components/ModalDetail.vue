      ...
      <NSpin :show="state.loading">
        <NDescriptions label-placement="left" bordered :column="1">
          ...
          <NDescriptionsItem :label="t('resource.message.notification.severity')">
            <NTag
              :color="createTagColor(dictTypeColor('NOTIFICATION_SEVERITY', state.detailData.severity))"
              :bordered="false"
            >
              {{ dictTypeData('NOTIFICATION_SEVERITY', state.detailData.severity) || displayValue(state.detailData.severity) }}
            </NTag>
          </NDescriptionsItem>
          ...
          <NDescriptionsItem :label="t('common.often.status')">
            <NTag
              ...
              :bordered="false"
            >
              {{ dictTypeData('NOTIFICATION_STATUS', state.detailData.status) || displayValue(state.detailData.status) }}
            </NTag>
          </NDescriptionsItem>
