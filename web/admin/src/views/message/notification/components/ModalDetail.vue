...
<template>
  <NModal v-model:show="state.showModal" preset="card" draggable :mask-closable="false" title="通知详情" style="width: min(760px, 94vw)">
      ...
      <NSpin :show="state.loading">
        <NDescriptions label-placement="left" bordered :column="1">
          ...
          <NDescriptionsItem label="分类">
            {{ dictTypeData('SYS_BIZ_CATEGORY', state.detail.category) || displayValue(state.detail.category) }}
          </NDescriptionsItem>
          ...
          <NDescriptionsItem label="内容格式">
            <NTag bordered="false">{{ dictTypeData('CONTENT_TYPE', state.detail.content_type) || displayValue(state.detail.content_type) }}</NTag>
          </NDescriptionsItem>
