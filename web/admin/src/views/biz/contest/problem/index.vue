...
import { ojContestProblemApi, ojContestApi } from '@/api'
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '竞赛内分值', path: 'points', width: 100 },
  {
    ...
    width: 80,
    render: row => (
      <NTag type={row.partial === true ? 'success' : 'default'} bordered={false}>
        {row.partial === true ? '是' : row.partial === false ? '否' : '-'}
      </NTag>
    ),
  },
  {
    ...
    width: 90,
    render: row => (
      <NTag type={row.is_pretested === true ? 'success' : 'default'} bordered={false}>
        {row.is_pretested === true ? '是' : row.is_pretested === false ? '否' : '-'}
      </NTag>
    ),
  },
  { title: '顺序', path: 'sort', width: 70 },
  { title: '更新时间', path: 'updated_at', width: 170, render: row => formatDateTime(row.updated_at) },
  {
    ...
    render: row => (
      <NFlex size={12}>
        {hasPermission('biz:contest:problem:detail') ? (
          <NButton type="info" size="small" text={true} onClick={() => openDetailModal(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('biz:contest:problem:update') ? (
          <NButton type="primary" size="small" text={true} onClick={() => openEditModal(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('biz:contest:problem:delete') ? (
...
async function fetchPage() {
  ...
  state.loading = true
  try {
    const response = await ojContestProblemApi.page(contestId.value, { current: state.page, size: state.pageSize, ...state.searchValues })
    const data = response.data ?? {}
...
async function deleteRows(ids: string[]) {
  await ojContestProblemApi.remove(contestId.value, { ids })
  ...
  <NFlex class="h-full min-h-0" vertical>
    <ProCard v-if="!props.embedded">
      <NFlex align="center" :size="12">
        ...
        <span class="font-medium">竞赛题目</span>
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton v-if="hasPermission('biz:contest:problem:create')" type="primary" text @click="openCreateModal">
          ...
          <NButton v-if="hasPermission('biz:contest:problem:delete')" type="error" text :disabled="!hasCheckedRows" @click="confirmDelete(state.checkedRowKeys)">
