...
const searchColumns = computed<ProSearchFormColumns<any>>(() => [
  { title: '标签编码', path: 'code', field: 'input' },
  { title: '标签名称', path: 'name', field: 'input' },
])
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '标签编码', path: 'code', width: 150, ellipsis: { tooltip: true } },
  { title: '标签名称', path: 'name', width: 150, ellipsis: { tooltip: true } },
  { title: '颜色', path: 'color', width: 150, ellipsis: { tooltip: true } },
  { title: '描述', path: 'description', width: 150, ellipsis: { tooltip: true } },
  { title: '更新时间', path: 'updated_at', width: 190, render: row => formatDateTime(row.updated_at) },
  {
    ...
    render: row => (
      <NFlex size={12}>
        {hasPermission('biz:contest:tag:detail') ? (
          <NButton type="info" size="small" text={true} onClick={() => openDetailModal(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('biz:contest:tag:update') ? (
          <NButton type="primary" size="small" text={true} onClick={() => openEditModal(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('biz:contest:tag:delete') ? (
...
async function fetchPage() {
  state.loading = true
  try {
    const response = await ojContestTagApi.page({ current: state.page, size: state.pageSize, ...state.searchValues })
...
async function deleteRows(ids: string[]) {
  await ojContestTagApi.remove({ ids })
...
<template>
  <NFlex class="h-full min-h-0" vertical>
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton v-if="hasPermission('biz:contest:tag:create')" type="primary" text @click="openCreateModal">
          ...
          <NButton v-if="hasPermission('biz:contest:tag:delete')" type="error" text :disabled="!hasCheckedRows" @click="confirmDelete(state.checkedRowKeys)">
