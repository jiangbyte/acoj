<!--
  ...
  Author: Charlie
  Generated at: 2026-07-19 10:54:15
-->
...
const searchColumns = computed<ProSearchFormColumns<any>>(() => [
  { title: '分类编码', path: 'code', field: 'input' },
  { title: '分类名称', path: 'name', field: 'input' },
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '父级ID', path: 'parent_id', width: 150, ellipsis: { tooltip: true } },
  { title: '分类编码', path: 'code', width: 150, ellipsis: { tooltip: true } },
  { title: '分类名称', path: 'name', width: 150, ellipsis: { tooltip: true } },
  {
    title: '状态',
    path: 'status',
    width: 150,
...
async function fetchPage() {
  state.loading = true
  try {
    const response = await cgTestKnowledgeCategoryApi.page({
      current: state.page,
      size: state.pageSize,
      ...state.searchValues,
    })
...
async function fetchChildPage() {
  ...
  try {
    const response = await cgTestKnowledgeCategoryApi.childPage({
      ...
      master_id: state.selectedMasterId,
      ...state.childSearchValues,
    })
    const data = response.data ?? {}
    state.childRows = data.records ?? []
    state.childTotal = data.total ?? 0
    state.childPage = data.current ?? state.childPage
    state.childPageSize = data.size ?? state.childPageSize
