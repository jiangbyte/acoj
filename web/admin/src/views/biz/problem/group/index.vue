...
const searchColumns = computed<ProSearchFormColumns<any>>(() => [
  { title: '分组编码', path: 'code', field: 'input' },
  { title: '分组名称', path: 'name', field: 'input' },
])
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '分组编码', path: 'code', width: 150, ellipsis: { tooltip: true } },
  { title: '分组名称', path: 'name', width: 150, ellipsis: { tooltip: true } },
  ...
  { title: '更新时间', path: 'updated_at', width: 190, render: row => formatDateTime(row.updated_at) },
  {
    ...
    render: row => (
      <NFlex size={12}>
        {hasPermission('biz:problem:group:detail') ? (
          <NButton type="info" size="small" text={true} onClick={() => openDetailModal(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:group:update') ? (
          <NButton type="primary" size="small" text={true} onClick={() => openEditModal(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:group:delete') ? (
...
async function fetchPage() {
  state.loading = true
  try {
    const response = await ojProblemGroupApi.page({ current: state.page, size: state.pageSize, ...state.searchValues })
...
async function deleteRows(ids: string[]) {
  await ojProblemGroupApi.remove({ ids })
...
<template>
  <NFlex class="h-full min-h-0" vertical>
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton v-if="hasPermission('biz:problem:group:create')" type="primary" text @click="openCreateModal">
          ...
          <NButton v-if="hasPermission('biz:problem:group:delete')" type="error" text :disabled="!hasCheckedRows" @click="confirmDelete(state.checkedRowKeys)">
