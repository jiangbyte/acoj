  ...
  >
    ...
    >
      <NGrid :cols="24" :x-gap="12">
        ...
        <NFormItemGi :span="12" label="数据模式" path="data_mode" :rule="{ required: true, message: '必选', trigger: ['change', 'blur'] }">
        ...
        <template v-if="isInline">
          <NFormItemGi :span="12" label="输入" path="input_inline" :rule="{ required: true, message: '请填写输入', trigger: ['input', 'blur'] }">
            <NFlex vertical :size="8" class="w-full">
              <NButton size="tiny" @click="inputFileRef?.click()">
                从本地文件载入
              </NButton>
              <input
                ref="inputFileRef"
          ...
          <NFormItemGi :span="12" label="输出" path="output_inline">
            <NFlex vertical :size="8" class="w-full">
              <NButton size="tiny" @click="outputFileRef?.click()">
                从本地文件载入
              </NButton>
              <input
                ref="outputFileRef"
              ...
              <NInput
                ...
                type="textarea"
                :rows="14"
                placeholder="output_inline（交互题可留空字符串）"
                class="font-mono"
              />
            </NFlex>
          </NFormItemGi>
        ...
        <template v-else>
          <NFormItemGi :span="12" label="输入文件" path="input_file" :rule="{ required: true, message: '必填', trigger: ['change', 'blur'] }">
            <FileUpload v-model:value="form.input_file" value-type="object_name" button-text="上传输入" />
          </NFormItemGi>
          <NFormItemGi :span="12" label="输出文件" path="output_file" :rule="{ required: true, message: '必填', trigger: ['change', 'blur'] }">
            <FileUpload v-model:value="form.output_file" value-type="object_name" button-text="上传输出" />
          </NFormItemGi>
        </template>
      </NGrid>
    </NForm>
