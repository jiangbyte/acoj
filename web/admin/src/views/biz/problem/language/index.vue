...
import { ojProblemLanguageApi, ojProblemApi } from '@/api'
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '语言标识', path: 'language_key', width: 120, ellipsis: { tooltip: true } },
  { title: '时间限制(ms)', path: 'time_limit_ms', width: 120, render: row => row.time_limit_ms ?? '默认' },
  { title: '内存(KB)', path: 'memory_limit_kb', width: 120, render: row => row.memory_limit_kb ?? '默认' },
  { title: '更新时间', path: 'updated_at', width: 190, render: row => formatDateTime(row.updated_at) },
  {
    ...
    render: row => (
      <NFlex size={12}>
        {hasPermission('biz:problem:language:detail') ? (
          <NButton type="info" size="small" text={true} onClick={() => openDetailModal(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:language:update') ? (
          <NButton type="primary" size="small" text={true} onClick={() => openEditModal(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:language:delete') ? (
...
async function fetchPage() {
  ...
  state.loading = true
  try {
    const response = await ojProblemLanguageApi.page(problemId.value, { current: state.page, size: state.pageSize, ...state.searchValues })
    const data = response.data ?? {}
...
async function deleteRows(ids: string[]) {
  await ojProblemLanguageApi.remove(problemId.value, { ids })
...
<template>
  <NFlex class="h-full min-h-0" vertical>
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton v-if="hasPermission('biz:problem:language:create')" type="primary" text @click="openCreateModal">
          ...
          <NButton v-if="hasPermission('biz:problem:language:delete')" type="error" text :disabled="!hasCheckedRows" @click="confirmDelete(state.checkedRowKeys)">
