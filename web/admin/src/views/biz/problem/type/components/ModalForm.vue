...
const modalTitle = computed(() => state.dataId ? '编辑题目类型' : '新增题目类型')
const rules = computed<FormRules>(() => ({
  code: [
    createRequiredRule('类型编码', 'input'),
  ],
  name: [
    createRequiredRule('类型名称', 'input'),
  ],
  sort: [
    {
...
async function fetchDetail(id: string) {
  state.loading = true
  try {
    const response = await ojProblemTypeApi.detail({ id })
...
async function submitForm() {
  ...
  try {
    ...
    if (state.dataId) {
      await ojProblemTypeApi.update({ ...payload, id: state.dataId })
      window.$message.success('更新成功')
    } else {
      await ojProblemTypeApi.create(payload)
    ...
    <NSpin :show="state.loading">
      <NScrollbar class="max-h-[min(620px,calc(100vh-300px))] pr-16px">
        <NForm ref="formRef" :model="state.formModel" :rules="rules" label-placement="left" label-width="110" :disabled="state.loading || state.submitLoading">
          <NFormItem label="类型编码" path="code">
