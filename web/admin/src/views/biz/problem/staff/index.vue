...
import { ojProblemStaffApi, ojProblemApi } from '@/api'
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '更新时间', path: 'updated_at', width: 190, render: row => formatDateTime(row.updated_at) },
  {
    ...
    render: row => (
      <NFlex size={12}>
        {hasPermission('biz:problem:staff:detail') ? (
          <NButton type="info" size="small" text={true} onClick={() => openDetailModal(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:staff:update') ? (
          <NButton type="primary" size="small" text={true} onClick={() => openEditModal(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:staff:delete') ? (
...
async function fetchPage() {
  ...
  state.loading = true
  try {
    const response = await ojProblemStaffApi.page(problemId.value, { current: state.page, size: state.pageSize, ...state.searchValues })
    const data = response.data ?? {}
...
async function deleteRows(ids: string[]) {
  await ojProblemStaffApi.remove(problemId.value, { ids })
  ...
  <NFlex :class="props.embedded ? 'min-h-0' : 'h-full min-h-0'" vertical>
    <ProCard v-if="!props.embedded">
      <NFlex align="center" :size="12">
        ...
        <span class="font-medium">题目人员</span>
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton v-if="hasPermission('biz:problem:staff:create')" type="primary" text @click="openCreateModal">
          ...
          <NButton v-if="hasPermission('biz:problem:staff:delete')" type="error" text :disabled="!hasCheckedRows" @click="confirmDelete(state.checkedRowKeys)">
