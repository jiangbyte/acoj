...
import { ojProblemSolutionApi, ojProblemApi } from '@/api'
...
const tableColumns = computed<ProDataTableColumns<any>>(() => [
  ...
  { title: '题解正文', path: 'content', width: 150, ellipsis: { tooltip: true } },
  {
    title: '是否公开',
    path: 'is_public',
    width: 120,
    render: row => (
      <NTag type={row.is_public === true ? 'success' : 'default'} bordered={false}>
        {row.is_public === true ? '是' : row.is_public === false ? '否' : '-'}
      </NTag>
    ),
  },
  { title: '公开时间', path: 'publish_at', width: 190, render: row => formatDateTime(row.publish_at) },
  { title: '作者账户ID列表', path: 'author_account_ids', width: 150, ellipsis: { tooltip: true } },
  { title: '更新时间', path: 'updated_at', width: 190, render: row => formatDateTime(row.updated_at) },
  {
    ...
    render: row => (
      <NFlex size={12}>
        {hasPermission('biz:problem:solution:detail') ? (
          <NButton type="info" size="small" text={true} onClick={() => openDetailModal(row.id)}>
            {renderButtonIcon('icon-park-outline:preview-open')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:solution:update') ? (
          <NButton type="primary" size="small" text={true} onClick={() => openEditModal(row.id)}>
            {renderButtonIcon('icon-park-outline:edit')}
          </NButton>
        ) : null}
        {hasPermission('biz:problem:solution:delete') ? (
...
async function fetchPage() {
  ...
  state.loading = true
  try {
    const response = await ojProblemSolutionApi.page(problemId.value, { current: state.page, size: state.pageSize, ...state.searchValues })
    const data = response.data ?? {}
...
async function deleteRows(ids: string[]) {
  await ojProblemSolutionApi.remove(problemId.value, { ids })
  ...
  <NFlex :class="props.embedded ? 'min-h-0' : 'h-full min-h-0'" vertical>
    <ProCard v-if="!props.embedded">
      <NFlex align="center" :size="12">
        ...
        <span class="font-medium">题解</span>
    ...
    >
      <template #toolbar>
        <NFlex>
          <NButton v-if="hasPermission('biz:problem:solution:create')" type="primary" text @click="openCreateModal">
          ...
          <NButton v-if="hasPermission('biz:problem:solution:delete')" type="error" text :disabled="!hasCheckedRows" @click="confirmDelete(state.checkedRowKeys)">
