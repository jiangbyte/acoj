...
function isOwnMessage(message: Message): boolean {
  ...
  return message.sender_account_id === user?.accountId
...
function handleFileInputChange(event: Event) {
  ...
  files.forEach((file, i) => { selectedFiles.value[startIndex + i] = file })
  selectedAttachments.value = [
    ...selectedAttachments.value,
    ...files.map((file) => ({ name: file.name, size: file.size, type: file.type || 'application/octet-stream', url: '' })),
  ]
  input.value = ''
}
...
async function sendMessage() {
  ...
  const pending = selectedAttachments.value.map((a, i) => ({ attachment: a, file: selectedFiles.value[i], index: i }))
    .filter(({ attachment, file }) => !attachment.url && file)
  if (pending.length) {
    for (const { attachment, file } of pending) {
      try {
        const res = await fileApi.upload(file)
        const data = res?.data as { id?: string; url?: string; name?: string; content_type?: string; size?: number } | undefined
        if (data) {
  ...
  try {
    const res = await messageApi.sendMessage({
      conversation_id: cid.value,
      content: content || ' ',
      attachments: selectedAttachments.value.length ? selectedAttachments.value.map((a: any) => ({
        file_id: a.file_id,
        name: a.name,
        url: a.url,
        size: a.size,
...
async function open(conversationId: string, info?: { title?: string; avatar?: string; conversationType?: string }) {
...
<template>
  <NModal v-model:show="show" preset="card" draggable :mask-closable="false" :title="modalTitle" style="width: 760px">
    <div class="h-[560px] flex min-h-0 flex-col -mx-16px -mb-16px">
      <!-- header: conversation info bar -->
      <div v-if="conversation" class="flex items-center gap-3 border-b px-4 py-2.5" :style="{ borderColor: themeVars.borderColor }">
        <NAvatar v-if="conversation.avatar" round :size="32" class="shrink-0" :src="resolveFileUrl(conversation.avatar)" :img-props="avatarImgProps" />
        <NAvatar v-else round :size="32" class="shrink-0">{{ (conversation.title || '会话').charAt(0) }}</NAvatar>
        <div class="min-w-0">
          <div class="text-sm font-600">{{ conversation.title }}</div>
          <div class="text-xs" :style="{ color: themeVars.textColor3 }">{{ conversationTypeLabel }}</div>
        </div>
      </div>
      ...
      <div class="flex min-h-0 flex-1 flex-col">
        <div v-if="hasMoreOlder" class="border-b px-4 py-2 text-center" :style="{ borderColor: themeVars.borderColor }">
          <NButton text size="small" :loading="loadingOlder" @click="loadOlderMessages">上滑加载更早消息</NButton>
        </div>
        <div v-if="loading && !visibleMessages.length" class="flex items-center justify-center py-12 text-sm" :style="{ color: themeVars.textColor3 }">
          加载中...
        </div>
        <div ref="messageListRef" class="flex min-h-0 flex-1 flex-col gap-3 overflow-auto px-4 py-4" @scroll.passive="handleMessageScroll">
          <div v-if="visibleMessages.length" class="flex flex-col gap-3">
            <div v-for="message in visibleMessages" :key="message.id" class="flex items-start gap-2"
              ...
              <NAvatar v-if="message.sender_avatar" round :size="28" class="shrink-0" :src="resolveFileUrl(message.sender_avatar)" :img-props="avatarImgProps" />
              <NAvatar v-else round :size="28" class="shrink-0">{{ message.sender_nickname?.charAt(0) || message.sender_name?.charAt(0) || '?' }}</NAvatar>
              <div class="min-w-0 max-w-[min(68%,640px)]">
                <div class="mb-1 flex gap-2 text-xs" :class="isOwnMessage(message) ? 'justify-end' : 'justify-start'" :style="{ color: themeVars.textColor3 }">
                  <span>{{ message.sender_nickname || message.sender_name || '未知' }}</span>
                  <span>{{ formatDateTime(message.created_at) }}</span>
                </div>
                <div class="rounded-2 px-3 py-2 text-sm leading-6" :style="messageBubbleStyle(isOwnMessage(message))">
                  ...
                  <div v-if="message.attachments?.length" class="mt-2 flex flex-col gap-2">
                    <template v-for="attachment in message.attachments" :key="attachment.id">
                      <div v-if="isImageFile(attachment)" class="group/image relative overflow-hidden rounded-1">
                        <NImage :src="resolveFileUrl(attachment.url)" :alt="attachment.name"
                          height="160" class="max-w-full" object-fit="cover"
                          :img-props="{ referrerPolicy: 'no-referrer', loading: 'lazy' }" />
                        <a :href="resolveFileUrl(attachment.url)" download :title="'下载 ' + attachment.name"
                      ...
                      <a v-else :href="resolveFileUrl(attachment.url)" download :title="attachment.name"
                        class="flex items-center gap-3 rounded-1 border px-3 py-2"
                        :class="isOwnMessage(message) ? 'border-white/20 bg-white/10' : ''"
                        :style="!isOwnMessage(message) ? { borderColor: themeVars.borderColor, backgroundColor: themeVars.bodyColor } : {}">
