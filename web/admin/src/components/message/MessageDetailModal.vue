...
<template>
  <NModal v-model:show="state.show" preset="card" draggable :mask-closable="false" title="通知 详情" style="width: 720px">
    <NScrollbar class="max-h-[min(620px,calc(100vh-300px))] pr-16px">
      <NSpin :show="state.loading">
        <NDescriptions label-placement="left" bordered :column="1">
          <NDescriptionsItem :label="'标题'">{{ displayValue(state.detail.title || state.source.title) }}</NDescriptionsItem>
          ...
          <NDescriptionsItem :label="'发布时间'">{{ formatDateTime(state.detail.publish_at || state.source.publish_at) }}</NDescriptionsItem>
          <NDescriptionsItem :label="'内容'">{{ displayValue(state.detail.content || state.source.content) }}</NDescriptionsItem>
        ...
        <NSpace class="mt-4" justify="end">
          <NButton v-if="!(state.detail.is_read || state.source.is_read)" type="primary" :loading="state.actionLoading" @click="markNotificationRead">标记已读</NButton>
        </NSpace>
      </NSpin>
    </NScrollbar>
  </NModal>
