/**
 * Watch a submission via fetch-based SSE (supports Authorization; EventSource cannot).
 */
...
function eventsUrl(submissionId: string, maxWaitSec: number) {
  const base = (import.meta.env.VITE_API_URL || '').replace(/\/$/, '')
  const qs = new URLSearchParams({
    id: submissionId,
    max_wait_sec: String(maxWaitSec),
  })
  return `${base}/api/v1/admin/biz/submission/submission/events?${qs}`
}
...
/** Subscribe until terminal status / timeout / abort. */
export async function watchSubmissionEvents(
  ...
  const response = await fetch(eventsUrl(submissionId, maxWaitSec), {
    method: 'GET',
    headers: {
...
/** Fallback poller if SSE unavailable. */
