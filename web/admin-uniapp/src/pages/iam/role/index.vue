<template>
  <Layout title="角色管理" back>
    <view class="flex flex-col">
      <view class="bg-white mx-4 mt-3 rounded-lg overflow-hidden">
        <view class="flex items-center gap-2 p-2">
          <u-search v-model="keyword" placeholder="搜索角色名称/编码" :show-action="false" @search="onSearch" class="flex-1" />
    ...
    <u-popup :show="formVisible" mode="bottom" :safe-area-inset-bottom="true" @close="formVisible = false">
      <view class="bg-white rounded-t-lg p-4" style="max-height: 70vh; overflow-y: auto;">
        <text class="block text-lg font-bold mb-4">{{ editingId ? '编辑角色' : '新增角色' }}</text>
        <u-form :model="form">
          <u-form-item label="角色编码" required><u-input v-model="form.code" placeholder="请输入编码" border="none" /></u-form-item>
          <u-form-item label="角色名称" required><u-input v-model="form.name" placeholder="请输入名称" border="none" /></u-form-item>
          <u-form-item label="分类"><view class="w-full" @click="openPicker('category', dictOptions('SYS_BIZ_CATEGORY'))"><u-input :value="dictLabel('SYS_BIZ_CATEGORY', form.category)" placeholder="请选择" disabled border="none" /></view></u-form-item>
          <u-form-item label="范围类型"><view class="w-full" @click="openPicker('scope_type', dictOptions('ROLE_SCOPE_TYPE'))"><u-input :value="dictLabel('ROLE_SCOPE_TYPE', form.scope_type)" placeholder="请选择" disabled border="none" /></view></u-form-item>
          <u-form-item label="排序"><u-number-box v-model="form.sort" :min="0" /></u-form-item>
          <u-form-item label="内置角色"><u-switch v-model="form.is_builtin" /></u-form-item>
          ...
          <u-form-item label="描述"><u-textarea v-model="form.description" placeholder="请输入描述" border="none" /></u-form-item>
        </u-form>
    ...
    <u-popup :show="detailVisible" mode="bottom" :safe-area-inset-bottom="true" @close="detailVisible = false">
      <view class="bg-white rounded-t-lg p-4" style="max-height: 70vh; overflow-y: auto;">
        <text class="block text-lg font-bold mb-4">角色详情</text>
...
const detailFields = ['id', 'code', 'name', 'category', 'scope_type', 'owner_dept_id', 'sort', 'is_builtin', 'status', 'description', 'created_at', 'updated_at']
...
async function loadPage(append: boolean) {
  ...
  try {
    ...
    const page = await roleApi.page(params)
...
function detailLabel(p: string) { const m: Record<string, string> = { id: 'ID', code: '编码', name: '名称', category: '分类', scope_type: '范围类型', owner_dept_id: '所属部门ID', sort: '排序', is_builtin: '内置角色', status: '状态', description: '描述', created_at: '创建时间', updated_at: '更新时间' }; return m[p] ?? p }
...
function openCreate() { editingId.value = ''; form.value = { sort: 99, status: 'ENABLED', is_builtin: false }; formVisible.value = true }
...
async function doSave() {
  ...
  try {
    ...
    if (editingId.value) { await roleApi.update({ ...payload, id: editingId.value }) } else { await roleApi.create(payload) }
    formVisible.value = false; await refresh()
  } finally { saving.value = false }
}

function confirmDelete(item: any) { uni.showModal({ title: '确认删除', content: `删除 ${item.name}？`, success: async (r: any) => { if (r.confirm) { await roleApi.remove({ ids: [item.id] }); await refresh() } } }) }
</script>
