...
export function ProblemListDetailPage() {
  ...
  const problems = useMemo(() => {
    ...
    if (!q) return rows
    return rows.filter((p: any) => p.name.toLowerCase().includes(q) || p.code.toLowerCase().includes(q))
  }, [detail, keyword])

  const progress = detail?.progress
  const pct = progress?.total ? Math.round((progress.solved / progress.total) * 100) : 0
  const firstUnsolved = problems.find((p: any) => !p.solved)
