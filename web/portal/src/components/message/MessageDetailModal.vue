        ...
        <template v-if="state.type === 'notification'">
          <NDescriptions label-placement="left" bordered :column="1">
            ...
            <NDescriptionsItem :label="t('common.often.status')">
              <NTag :type="state.detail.is_read || state.source.is_read ? 'success' : 'warning'" :bordered="false">
                {{ state.detail.is_read || state.source.is_read ? t('app.notice.read') : t('app.notice.unread') }}
              </NTag>
            </NDescriptionsItem>
        ...
        <template v-else-if="state.type === 'message'">
          <NDescriptions label-placement="left" bordered :column="1">
            ...
            <NDescriptionsItem :label="t('resource.message.message.thread_type')">
              <NTag
                :color="createTagColor(dictTypeColor('MESSAGE_THREAD_TYPE', state.detail.thread_type || state.source.thread_type))"
                :bordered="false"
              >
                {{ dictTypeData('MESSAGE_THREAD_TYPE', state.detail.thread_type || state.source.thread_type) || displayValue(state.detail.thread_type || state.source.thread_type) }}
              </NTag>
            </NDescriptionsItem>
            <NDescriptionsItem :label="t('common.often.updated_at')">
          ...
          <NList v-if="state.messages.length" bordered>
            <NListItem v-for="item in state.messages" :key="item.id">
              <NThing>
                <template #header>
                  {{ item.sender_name || item.sender_account_id || t('resource.message.message.system_sender') }}
                </template>
          ...
          <NDescriptions label-placement="left" bordered :column="1">
            ...
            <NDescriptionsItem :label="t('resource.message.todo.priority')">
              <NTag
                ...
                :bordered="false"
              >
                {{ dictTypeData('TODO_PRIORITY', state.detail.priority) || displayValue(state.detail.priority) }}
              </NTag>
            </NDescriptionsItem>
            <NDescriptionsItem :label="t('common.often.status')">
              <NTag
                ...
                :bordered="false"
              >
                {{ dictTypeData('TODO_STATUS', state.detail.status) || displayValue(state.detail.status) }}
              </NTag>
            </NDescriptionsItem>
            <NDescriptionsItem :label="t('resource.message.todo.assignee_status')">
              <NTag
                ...
                :bordered="false"
              >
                {{ dictTypeData('TODO_STATUS', todoAssigneeStatus()) || displayValue(todoAssigneeStatus()) }}
              </NTag>
            </NDescriptionsItem>
