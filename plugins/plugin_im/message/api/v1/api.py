"""
Message API routes — sys (admin) only for now.
...
async def conversation_read_handler(request: Request, p: dict):
    ...
    if cid.startswith("group:"):
        group_mark_read(cid[6:], uid, "BUSINESS")
    else:
...
@HeiCheckLogin
async def get_or_create_conversation_handler(request: Request, p: GetOrCreateConversationParam):
    uid, ut = await _sys_user(request)
...
@HeiCheckLogin
async def file_upload_handler(request: Request, file: UploadFile = File(...)):
    uid, ut = await _sys_user(request)
    try:
...
@HeiClientCheckLogin
async def client_unread_count_handler(request: Request):
...
@HeiClientCheckLogin
async def client_file_upload_handler(request: Request, file: UploadFile = File(...)):
    uid, ut = await _client_user(request)
    try:
        result = await upload_file(file, uid, ut)
        return success(result.__dict__)
