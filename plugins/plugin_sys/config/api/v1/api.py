...
async def page(request: Request, param: ConfigPageParam = Depends(), db: Session = Depends(get_db)):
...
async def list_by_category(request: Request, param: ConfigListParam = Depends(), db: Session = Depends(get_db)):
    service = ConfigService(db)
...
async def create(request: Request, vo: ConfigVO, db: Session = Depends(get_db)):
    service = ConfigService(db)
...
async def modify(request: Request, vo: ConfigVO, db: Session = Depends(get_db)):
    service = ConfigService(db)
...
@NoRepeat(interval=3000)
async def edit_batch(request: Request, param: ConfigBatchEditParam, db: Session = Depends(get_db)):
    service = ConfigService(db)
...
@HeiCheckPermission("sys:config:edit")
@NoRepeat(interval=3000)
async def edit_by_category(request: Request, param: ConfigCategoryEditParam, db: Session = Depends(get_db)):
    service = ConfigService(db)
