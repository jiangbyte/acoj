...
from sqlalchemy.dialects.mysql import DATE, DATETIME, INTEGER, LONGTEXT, TEXT, VARCHAR
...
class SysUser(Base):
    ...
    status: Mapped[Optional[str]] = mapped_column(VARCHAR(16, charset='utf8mb4', collation='utf8mb4_general_ci'), default="ACTIVE", server_default=text("'ACTIVE'"), comment='状态')
    ...
    login_count: Mapped[Optional[int]] = mapped_column(INTEGER, default=0, server_default=text("0"), comment='登录次数')
...
class RelUserPermission(Base):
    ...
    scope: Mapped[Optional[str]] = mapped_column(VARCHAR(32, charset='utf8mb4', collation='utf8mb4_general_ci'), default="ALL", server_default=text("'ALL'"), comment='数据范围：ALL-全部，SELF-本人，ORG-本组织，ORG_AND_BELOW-本组织及以下，CUSTOM_ORG-自定义组织，GROUP-本用户组，GROUP_AND_BELOW-本用户组及以下，CUSTOM_GROUP-自定义用户组')
