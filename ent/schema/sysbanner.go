...
func (SysBanner) Annotations() []schema.Annotation {
	return []schema.Annotation{
		entsql.Annotation{Table: "sys_banner"},
	}
}
...
func (SysBanner) Fields() []ent.Field {
	return []ent.Field{
		...
		field.String("title").MaxLen(255).NotEmpty().Comment("轮播标题"),
		field.String("image").MaxLen(500).NotEmpty().Comment("轮播图片"),
		field.String("url").MaxLen(500).Optional().Nillable().Comment("跳转地址"),
		field.String("link_type").MaxLen(16).Optional().Default("URL").Comment("链接类型"),
		field.String("summary").MaxLen(500).Optional().Nillable().Comment("轮播摘要"),
		field.Text("description").Optional().Nillable().Comment("轮播描述"),
		field.String("category").MaxLen(32).NotEmpty().Comment("轮播类别"),
		field.String("type").MaxLen(32).NotEmpty().Comment("轮播类型"),
		field.String("position").MaxLen(32).NotEmpty().Comment("展示位置"),
