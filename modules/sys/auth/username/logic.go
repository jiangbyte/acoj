...
// DoLogin handles the username-password login flow.
func DoLogin(c *gin.Context) {
	...
	_ = db.Client.SysUser.UpdateOneID(user.ID).
		SetLastLoginAt(now).
		SetLastLoginIP(ip).
		AddLoginCount(1).
		Exec(ctx)
...
// DoRegister handles the username-password registration flow.
...
// DoLogout handles the logout flow.
func DoLogout(c *gin.Context) {
	...
	if userID != "" {
		...
		if err == nil && user != nil {
			username := safeStr(user.Username)
			log.RecordAuthLog(c, "退出登录", "LOGOUT", "SUCCESS", "", username)
		}
	}
