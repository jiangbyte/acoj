...
class AnalyzeDao:
    ...
    def count_users(self) -> int:
        stmt = select(func.count()).select_from(SysUser).where(SysUser.is_deleted == 'NO')
    ...
    def count_active_users(self) -> int:
        stmt = (
            select(func.count())
            .select_from(SysUser)
            .where(SysUser.is_deleted == 'NO', SysUser.status == 'ACTIVE')
        )
        return self.db.execute(stmt).scalar() or 0

    def count_roles(self) -> int:
        stmt = select(func.count()).select_from(SysRole).where(SysRole.is_deleted == 'NO')
        return self.db.execute(stmt).scalar() or 0

    def count_orgs(self) -> int:
        stmt = select(func.count()).select_from(SysOrg).where(SysOrg.is_deleted == 'NO')
        return self.db.execute(stmt).scalar() or 0

    def count_configs(self) -> int:
        stmt = select(func.count()).select_from(SysConfig).where(SysConfig.is_deleted == 'NO')
        return self.db.execute(stmt).scalar() or 0

    def count_notices(self) -> int:
        stmt = select(func.count()).select_from(SysNotice).where(SysNotice.is_deleted == 'NO')
        return self.db.execute(stmt).scalar() or 0
    ...
    def user_trend(self, months: int = 12) -> List[dict]:
        stmt = text("""
            ...
            FROM sys_user
            WHERE is_deleted = 'NO' AND created_at IS NOT NULL
            GROUP BY month
            ORDER BY month ASC
            LIMIT :limit
        """)
    ...
    def org_user_distribution(self) -> List[dict]:
        stmt = text("""
            ...
            FROM sys_org o
            LEFT JOIN sys_user u ON u.org_id = o.id AND u.is_deleted = 'NO'
            WHERE o.is_deleted = 'NO'
