...
func Page(c *gin.Context, param *OrgPageParam) gin.H {
	...
	query := db.DB.WithContext(ctx).Model(&SysOrg{})
	if param.ParentID != "" {
		if param.ParentID == "0" {
			query = query.Where("(parent_id IS NULL OR parent_id = '' OR id = ?)", param.ParentID)
		} else {
			query = query.Where("(parent_id = ? OR id = ?)", param.ParentID, param.ParentID)
		}
	}
	if param.Keyword != "" {
