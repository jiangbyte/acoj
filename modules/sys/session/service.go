...
func Page(page, size int) (int, []SessionVO, error) {
	...
	pattern := constants.SessionPrefixBusiness + "*"
	...
	sessionPrefixLen := len(constants.SessionPrefixBusiness)
...
func Analysis() (*SessionAnalysisResult, error) {
	...
	// Business session analysis
	iter := db.Redis.Scan(ctx, 0, constants.SessionPrefixBusiness+"*", 0).Iterator()
	for iter.Next(ctx) {
		...
		for _, token := range tokens {
			tokenKey := constants.TokenPrefixBusiness + token
	...
	proportion := fmt.Sprintf("%d/%d", businessCount, consumerCount)
...
func TokenList(userID string) ([]TokenInfo, error) {
	ctx := context.Background()
	sessionKey := constants.SessionPrefixBusiness + userID
...
func ChartData() (*LogBarChartData, *LogPieChartData, error) {
	...
	pieData := &LogPieChartData{
		Data: []LogCategoryTotal{
			{Category: "B端", Total: businessTotal},
