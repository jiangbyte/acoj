...
func Conversations(currentUserID string) []ConversationVO {
	...
	rows1 := fetchConversationRows("client_message", currentUserID, "")
	for _, r := range rows1 {
		addConv(r)
	}
	rows2 := fetchConversationRowsSender("client_message", currentUserID, string(enums.LoginTypeConsumer))
	for _, r := range rows2 {
		addConv(r)
	}
	rows3 := fetchConversationRowsSender("sys_message", currentUserID, string(enums.LoginTypeBusiness))
	for _, r := range rows3 {
		addConv(r)
	}
	...
	if len(consumerIDs) > 0 {
		var conUsers []clientUser.ClientUser
		db.DB.Model(&clientUser.ClientUser{}).Where("id IN ?", consumerIDs).Find(&conUsers)
...
// ConversationMessages returns messages in a conversation for a CONSUMER user.
// Uses UNION to query both client_message and sys_message.
func ConversationMessages(currentUserID string, conversationID string, cursor string, size int) ([]ConversationMessageVO, bool) {
	if size < 1 {
		size = 20
	}
	if size > 100 {
