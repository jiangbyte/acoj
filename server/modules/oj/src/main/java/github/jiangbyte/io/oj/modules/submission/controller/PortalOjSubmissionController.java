...
public class PortalOjSubmissionController {
    ...
    public ApiResponse<OjSubmission> create(@Valid @RequestBody OjSubmissionCreateParam param) {
        ...
        return ApiResponse.ok(ojSubmissionService.createForPortal(param));
    }
    ...
    public ApiResponse<OjSubmission> detail(@Valid @ModelAttribute IdParam param) {
        StpKit.PORTAL.checkLogin();
        return ApiResponse.ok(ojSubmissionService.portalDetail(param.getId()));
    }
    ...
    public ApiResponse<Page<OjSubmission>> page(@Valid @ModelAttribute OjSubmissionPageParam param) {
        StpKit.PORTAL.checkLogin();
        // 门户强制本人数据，忽略入参 accountId
        param.setAccountId(null);
        return ApiResponse.ok(ojSubmissionService.portalPage(param));
    }

    /**
     * 更新本人提交备注。
     */
    @Operation(summary = "更新本人提交备注。")
    @PostMapping("/v1/portal/oj/submissions/update-note")
    @OperationAudit(resourceType = "oj_submission", action = "update_note")
    public ApiResponse<OjSubmission> updateNote(@Valid @RequestBody OjSubmissionUpdateNoteParam param) {
        StpKit.PORTAL.checkLogin();
        return ApiResponse.ok(ojSubmissionService.updateNoteForPortal(param));
    }
}
