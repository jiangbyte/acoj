...
```python
from sdk.log.utils import generate_log_signature
...
| 文件 | 说明 |
|------|------|
| `sdk/log/decorator.py` | @SysLog 装饰器 + save_exception_log 函数 |
| `sdk/log/utils.py` | User-Agent 解析、参数序列化、签名生成 |
| `plugins/sys/log/` | 日志查询管理模块（API + Service + Repository） |
