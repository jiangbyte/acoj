...
| 项 | 要求 |
|----|------|
| 网络 | SparkSandbox **仅内网**；安全组只允许 Worker/探活源访问 `8080`（或实际端口） |
| 沙箱容器 | `--privileged --cgroupns=host`（见 SparkSandbox 部署说明） |
| ACOJ | API 与 Worker 可同进程或拆分；Worker 可水平扩展 |
| 判题队列 | **RabbitMQ**（durable + confirm + manual ack）；**不用 Redis 做判题队列** |
| 执行机登记 | 生产节点必须写入 `oj_judge_node`；禁止只配单个公网 URL |
| 签名 | 生产 `signing_enabled=true`（节点级或全局） |

本地开发允许：单节点 + 关闭验签；文档与配置 profile 区分 `dev` / `prod`。
...
| 秘密 | 存放 | 注意 |
|------|------|------|
| 节点 HMAC | `oj_judge_node.signing_secret_cipher` | 使用现有配置加解密能力；解密仅在内存 |
| 全局默认 HMAC | `sys_config`：`oj.judge.default_signing_secret_cipher` | 节点密文为空时回落 |
...
| 指标 | 目标 |
|------|------|
| 探活失败 → 标记 `OFFLINE` | ≤ `3 × heartbeat_interval_ms`（默认 15s） |
...
| Job | 锁 | 周期 | 行为 |
|-----|-----|------|------|
| Heartbeat | ShedLock/JDBC 或 sys_job | 5s | 探活 |
...
| # | 场景 | 期望 |
|---|------|------|
| 1 | `kill -9` 沙箱进程 | 探活 OFFLINE / 传输失败换机；提交最终有结果或等待后 SE |
| 2 | 对沙箱断网 | 同换机；熔断 OPEN |
...
| 版本 | 说明 |
|------|------|
| P0 | 队列改为 RabbitMQ；在途仅 DB；单例用 JDBC/sys_job |
