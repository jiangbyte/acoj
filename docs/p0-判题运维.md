...
| Job | 锁 | 周期 | 行为 | 落地 |
|-----|-----|------|------|------|
| Heartbeat Stale | `sys_job` + Lock4j | FIXED 15s | 超时未心跳 → OFFLINE；推进半开 | `OjJudgeHeartbeatJob` |
| Lease Reaper | 同上 | FIXED 15s | 回收僵尸 JUDGING；dispatch CAS 成功才减 inflight | `OjJudgeLeaseReaperJob` |
| Inflight Reconcile | 同上 | FIXED 60s | JUDGING 聚合修复 `inflight_count` | `OjJudgeInflightReconcileJob` |
| Pending Enqueue | 同上 | FIXED 60s | PENDING 占位限频补偿 publish | `OjJudgePendingEnqueueJob` |
| Dispatch Purge | sys_job | 日 | 审计表清理 | （另任务） |

种子：`server/scripts/oj_p0_insert_judge_reconcile_jobs.sql`。索引核对：`oj_p0_ensure_submission_schedule_indexes.sql`。

对账修复原则：以 **`status=JUDGING` 按 node 聚合计数** 为权威修正 `oj_judge_node.inflight_count`；偏差写日志（后续可接指标）。队列积压看 RabbitMQ 管理指标；**不为判题队列使用 Redis**。
---
