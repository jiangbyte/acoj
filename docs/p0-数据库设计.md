...
发布为 `PUBLISHED` 前须满足：当前 `case_version` ≥1 条 ENABLED 测例、≥1 条 ENABLED 参考答案、存在一次 `mode=ALL` 且 `limit_mode=PROBLEM` 且整单 `AC` 的试跑记录。
...
| `oj_problem_dry_run.overall_status` | 同提交终态：`AC` / `WA` / … / `SE` |
...
`SUCCESS_RESULT` / `TRANSPORT_FAIL` / `SANDBOX_INTERNAL` / `CANCELLED_LEASE` / `TIMEOUT`
...
CREATE TABLE `oj_problem` (
  ...
  `allowed_languages` json NOT NULL COMMENT '允许语言 key 数组，如 ["cpp17","python3"]',
