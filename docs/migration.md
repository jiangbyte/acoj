...
async def main():
    ...
    await conn.execute("DROP DATABASE IF EXISTS acoj_migration_shadow")
    await conn.execute("CREATE DATABASE acoj_migration_shadow")
    await conn.close()

asyncio.run(main())
PY

DB__URL=postgresql+asyncpg://postgres:123456@127.0.0.1:5432/acoj_migration_shadow \
