...
func Remove(ctx context.Context, ids []string) error {
	...
	count, _ := dao.SysOrg.Ctx().Ctx(ctx).Where("parent_id in (?)", ids).Count()
	if count > 0 {
		return errors.New("组织存在子级，无法删除")
	}
	// Check for linked users
	count, _ = dao.RelOrgRole.Ctx().Ctx(ctx).Where("org_id in (?)", ids).Count()
	if count > 0 {
		return errors.New("组织存在关联数据，无法删除")
	}

	_, err := dao.SysOrg.Ctx().Ctx(ctx).WherePri(ids).Delete()
	return err
}
...
func Tree(ctx context.Context) ([]g.Map, error) {
	rows, err := dao.SysOrg.Ctx().Ctx(ctx).OrderAsc("sort_code").All()
	if err != nil {
		return nil, err
	}
